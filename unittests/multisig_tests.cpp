...
public:
   ...
   aacio_msig_tester() {
      create_accounts( { N(aacio.msig), N(aacio.stake), N(aacio.ram), N(aacio.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      produce_blocks();
      const auto& accnt = control->db().get<account_object,by_name>( N(aacio.msig) );
      abi_def abi;
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(aacio), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(aacio), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(aacio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const string& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(aacio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(aacio.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(aacio.msig), name, accounts, data );
      produce_block();
...
transaction aacio_msig_tester::reqauth( account_name from, const vector<permission_level>& auths, const fc::microseconds& max_serialization_time ) {
   fc::variants v;
...
BOOST_FIXTURE_TEST_CASE( propose_approve_execute, aacio_msig_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(exec), mvo()
                            ...
                            aacio_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, aacio_msig_tester ) try {

   // required to set up the link between (aacio active) and (aacio.prods active)
   //
   //                  aacio active
   //                       |
   //             aacio.prods active (2/3 threshold)
   //             /         |        \             <--- implicitly updated in onblock action
   // alice active     bob active   carol active

   set_authority(N(aacio), "active", authority(1,
      vector<key_weight>{{get_private_key("aacio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(aacio.prods), config::active_name}, 1}}), "owner",
      { { N(aacio), "active" } }, { get_private_key( N(aacio), "active" ) });

   set_producers( {N(alice),N(bob),N(carol)} );
   produce_blocks(50);

   create_accounts( { N(aacio.token) } );
   set_code( N(aacio.token), aacio_token_wast );
   set_abi( N(aacio.token), aacio_token_abi );

   create_currency( N(aacio.token), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
                        get_balance("aacio") + get_balance("aacio.ramfee") + get_balance("aacio.stake") + get_balance("aacio.ram") );
   ...
   create_account_with_resources( N(alice1111111), N(aacio), core_from_string("1.0000"), false );
   create_account_with_resources( N(bob111111111), N(aacio), core_from_string("0.4500"), false );
   create_account_with_resources( N(carol1111111), N(aacio), core_from_string("1.0000"), false );
   ...
   vector<permission_level> action_perm = {{N(aacio), config::active_name}};
   ...
   // execute by alice to replace the aacio system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect([&]( const transaction_trace_ptr& t) { if (t->scheduled) { trace = t; } } );
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(aacio), core_from_string("1.0000"), false ),
                            aacio_assert_message_exception, aacio_assert_message_is("Unknown Test")

   );
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, aacio_msig_tester ) try {

   // set up the link between (aacio active) and (aacio.prods active)
   set_authority(N(aacio), "active", authority(1,
      vector<key_weight>{{get_private_key("aacio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(aacio.prods), config::active_name}, 1}}), "owner",
      { { N(aacio), "active" } }, { get_private_key( N(aacio), "active" ) });
   ...
   create_currency( N(aacio.token), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( "aacio" ) );
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      aacio_assert_message_exception, aacio_assert_message_is("transaction authorization failed")
   );
