...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(aacio.token), N(alice)};
   test.init();
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(aacio.token), N(bob)};
   test.init();
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(aacio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_aacio ) { try {
   ...
   tester1.chain->set_code(config::system_account_name, aacio_token_wast);
   tester1.chain->produce_blocks();
   tester1.shutdown();
