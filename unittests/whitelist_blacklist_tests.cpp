...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(aacio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(aacio.token), aacio_token_wast);
         chain->set_abi(N(aacio.token), aacio_token_abi);
         chain->push_action( N(aacio.token), N(create), N(aacio.token), mvo()
              ( "issuer", "aacio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(aacio.token), N(issue), N(aacio.token), mvo()
              ( "to", "aacio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(aacio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {N(aacio), N(aacio.token), N(alice)};
   test.init();

   test.transfer( N(aacio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {N(aacio), N(aacio.token), N(bob)};
   test.init();

   test.transfer( N(aacio.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(aacio.token) );
   ...
   test.chain->set_code(N(bob), aacio_token_wast);
   test.chain->set_abi(N(bob), aacio_token_abi);

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), aacio_token_wast);
   test.chain->set_abi(N(charlie), aacio_token_abi);
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {N(aacio), N(aacio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_aacio ) { try {
   whitelist_blacklist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code(N(aacio), aacio_token_wast);
   tester1.chain->produce_blocks();
   tester1.shutdown();
   tester1.contract_blacklist = {N(aacio)};
   tester1.init(false);
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   tester1.action_blacklist = {{N(aacio), N(onerror)}};
   tester1.init(false);
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'aacio::onerror' is on the action blacklist")
                        );
