...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for aacio reqauth action with alice's spending key
   chain.link_authority("alice", "aacio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "aacio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with aacio reqauth
   chain.unlink_authority("alice", "aacio", "reqauth");
   ...
   // Link authority for any aacio action with alice's scud key
   chain.link_authority("alice", "aacio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "aacio", "first",  "reqauth");
...
try {
   ...
   // Creating account with aacio. prefix with privileged account
   chain.create_account("aacio.test1");

   // Creating account with aacio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("aacio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'aacio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "aacio", "aacio.any", "reqauth" );
   chain.link_authority( "bob", "aacio", "aacio.any", "reqauth" );

   /// this should succeed because aacio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = aacio::chain::resource_limits::resource_usage_object;
   using by_owner = aacio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link aacio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
