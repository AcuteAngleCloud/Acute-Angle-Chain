...
auto get_resolver(const abi_def& abi = abi_def())
{
   return [&abi](const account_name &name) -> optional<abi_serializer> {
      return abi_serializer(aacio_contract_abi(abi), max_serialization_time);
   };
}
...
{ try {
   ...
   abi_serializer abis(aacio_contract_abi(abi), max_serialization_time);
...
using namespace aacio::unittests::config;
...
struct abi_gen_helper {
  ...
  bool generate_abi(const char* source, const char* abi, bool opt_sfs=false) {
    ...
    bool res = runToolOnCodeWithArgs(
      new find_aacio_abi_macro_action(contract, actions, ""),
      source,
      extra_args      
    );
    FC_ASSERT(res == true);
...
BOOST_FIXTURE_TEST_CASE(abigen_aacioabi_macro, abi_gen_helper)
{ try {

   const char* abigen_aacioabi_macro = R"=====(
   ...
   const char* abigen_aacioabi_macro_abi = R"=====(
   {
     "types": [],
     "structs": [{
         "name": "hi",
   ...
   BOOST_TEST( generate_abi(abigen_aacioabi_macro, abigen_aacioabi_macro_abi) == true );
...
BOOST_FIXTURE_TEST_CASE(abigen_no_aacioabi_macro, abi_gen_helper)
{ try {

   const char* abigen_no_aacioabi_macro = R"=====(
      #pragma GCC diagnostic push
      #pragma GCC diagnostic ignored "-Wpointer-bool-conversion"
      #include <aaciolib/aacio.hpp>
      #include <aaciolib/print.hpp>
   ...
   const char* abigen_no_aacioabi_macro_abi = R"=====(
   {
     "version": "aacio::abi/1.0",
     "types": [],
     "structs": [{
   ...
   BOOST_TEST( generate_abi(abigen_no_aacioabi_macro, abigen_no_aacioabi_macro_abi) == true );
...
{ try {
   ...
   auto abi = aacio_contract_abi(fc::json::from_string(my_abi).as<abi_def>());

   abi_serializer abis(abi, max_serialization_time);

   const char *my_other = R"=====(
    {
      "publickey"     :  "AAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
      "publickey_arr" :  ["AAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","AAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","AAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"],
      "asset"         : "100.0000 SYS",
      ...
      "keyweight": {"key":"AAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"100"},
      "keyweight_arr": [{"key":"AAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"100"},{"key":"AAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"200"}],
      "authority": {
         "threshold":"10",
         "keys":[{"key":"AAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":100},{"key":"AAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":200}],
...
{ try {
   ...
   auto abi = aacio_contract_abi(fc::json::from_string(typedef_cycle_abi).as<abi_def>());
...
{ try {
   ...
   abi_serializer abis(aacio_contract_abi(abi_def()), max_serialization_time);
...
{ try {
   ...
   BOOST_TEST("AAC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)updauth.auth.keys[0].key);
   BOOST_TEST(57005u == updauth.auth.keys[0].weight);
   BOOST_TEST("AAC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)updauth.auth.keys[1].key);
...
{ try {
   ...
   BOOST_TEST("AAC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)newacct.owner.keys[0].key);
   BOOST_TEST(57005u == newacct.owner.keys[0].weight);
   BOOST_TEST("AAC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)newacct.owner.keys[1].key);
   ...
   BOOST_TEST("AAC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)newacct.active.keys[0].key);
   BOOST_TEST(57005u == newacct.active.keys[0].weight);
   BOOST_TEST("AAC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)newacct.active.keys[1].key);
...
{ try {
   ...
   abi_serializer abis(aacio_contract_abi(v.as<abi_def>()), max_serialization_time);
...
{ try {
   ...
   auto abi = aacio_contract_abi(fc::json::from_string(repeat_abi).as<abi_def>());
...
BOOST_AUTO_TEST_CASE(abi_account_name_in_aacio_abi)
{ try {
   // inifinite loop in types
   const char* repeat_abi = R"=====(
   {
...
BOOST_AUTO_TEST_CASE(abi_large_array)
{
   try {
      ...
      // curl http://127.0.0.1:8888/v1/chain/abi_bin_to_json -X POST -d '{"code":"aacio", "action":"hi", "binargs":"ffffffff08"}'
...
BOOST_AUTO_TEST_CASE(abi_recursive_structs)
{
   try {
      ...
      abi_serializer abis(fc::json::from_string(abi_str).as<abi_def>(), max_serialization_time);
      string hi_data = "{\"user\":\"aacio\",\"arg2\":{\"user\":\"1\"}}";
