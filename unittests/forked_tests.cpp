...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(aacio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(aacio.token), aacio_token_wast );
   c.set_abi( N(aacio.token), aacio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(aacio.token), N(create), N(aacio.token), mutable_variant_object()
              ("issuer",       "aacio" )
   ...
   cr = c.push_action( N(aacio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
