...
public:
   ...
   fc::variant get_global_state() {
    ...
    auto buyram( name payer, name receiver, asset ram ) {
       auto r = base_tester::push_action(N(aacio), N(buyram), payer, mvo()
                    ("payer", payer)
                    ("receiver", receiver)
                    ("quant", ram)
                    );
    ...
    auto delegate_bandwidth( name from, name receiver, asset net, asset cpu, uint8_t transfer = 1) {
       auto r = base_tester::push_action(N(aacio), N(delegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    auto register_producer(name producer) {
       auto r = base_tester::push_action(N(aacio), N(regproducer), producer, mvo()
    ...
    auto undelegate_bandwidth( name from, name receiver, asset net, asset cpu ) {
       auto r = base_tester::push_action(N(aacio), N(undelegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(aacio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create aacio.msig and aacio.token
        create_accounts({N(aacio.msig), N(aacio.token), N(aacio.ram), N(aacio.ramfee), N(aacio.stake), N(aacio.vpay), N(aacio.bpay), N(aacio.saving) });

        // Set code for the following accounts:
        //  - aacio (code: aacio.bios) (already set by tester constructor)
        //  - aacio.msig (code: aacio.msig)
        //  - aacio.token (code: aacio.token)
        set_code_abi(N(aacio.msig), aacio_msig_wast, aacio_msig_abi);//, &aacio_active_pk);
        set_code_abi(N(aacio.token), aacio_token_wast, aacio_token_abi); //, &aacio_active_pk);

        // Set privileged for aacio.msig and aacio.token
        set_privileged(N(aacio.msig));
        set_privileged(N(aacio.token));

        // Verify aacio.msig and aacio.token is privileged
        const auto& aacio_msig_acc = get<account_object, by_name>(N(aacio.msig));
        BOOST_TEST(aacio_msig_acc.privileged == true);
        const auto& aacio_token_acc = get<account_object, by_name>(N(aacio.token));
        BOOST_TEST(aacio_token_acc.privileged == true);


        // Create SYS tokens in aacio.token, set its manager as aacio
        ...
        create_currency(N(aacio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to aacio.system
        issue(N(aacio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           auto r = buyram(N(aacio), a.aname, asset(ram));
           BOOST_REQUIRE( !r->except_ptr );

           r = delegate_bandwidth(N(aacio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        auto votepro = [&]( account_name voter, vector<account_name> producers ) {
          ...
          base_tester::push_action(N(aacio), N(voteproducer), voter, mvo()
                                ("voter",  name(voter))
                                ("proxy", name(0) )
                                ("producers", producers)
                     );
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "aacio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), aacio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), aacio_assert_message_exception);
