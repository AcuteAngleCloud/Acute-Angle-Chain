...
#include "aacio_system_tester.hpp"

using namespace aacio_system;

BOOST_AUTO_TEST_SUITE(aacio_system_tests)

BOOST_FIXTURE_TEST_CASE( buysell, aacio_system_tester ) try {

   BOOST_REQUIRE_EQUAL( core_from_string("0.0000"), get_balance( "alice1111111" ) );

   transfer( "aacio", "alice1111111", core_from_string("1000.0000"), "aacio" );
   BOOST_REQUIRE_EQUAL( success(), stake( "aacio", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000") ) );

   auto total = get_total_stake( "alice1111111" );
   auto init_bytes =  total["ram_bytes"].as_uint64();

   const asset initial_ram_balance = get_balance(N(aacio.ram));
   const asset initial_ramfee_balance = get_balance(N(aacio.ramfee));
   ...
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_from_string("199.0000"), get_balance(N(aacio.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_from_string("1.0000"), get_balance(N(aacio.ramfee)) );
   ...
   transfer( "aacio", "alice1111111", core_from_string("100000000.0000"), "aacio" );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, aacio_system_tester ) try {
   ...
   const auto init_aacio_stake_balance = get_balance( N(aacio.stake) );
   ...
   BOOST_REQUIRE_EQUAL( init_aacio_stake_balance + core_from_string("300.0000"), get_balance( N(aacio.stake) ) );
   ...
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_from_string("1000.0000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( init_aacio_stake_balance, get_balance( N(aacio.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, aacio_system_tester ) try {
   cross_15_percent_threshold();

   issue( "aacio", core_from_string("1000.0000"), config::system_account_name );
   issue( "aacio.stake", core_from_string("1000.0000"), config::system_account_name );
   BOOST_REQUIRE_EQUAL( core_from_string("0.0000"), get_balance( "alice1111111" ) );

   //aacio stakes for alice with transfer flag

   transfer( "aacio", "bob111111111", core_from_string("1000.0000"), "aacio" );
   ...
   transfer( "aacio", "alice1111111", core_from_string("1000.0000"), "aacio" );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000") ) );
   //now alice's stake should be equal to transfered from aacio + own stake
   total = get_total_stake("alice1111111");
   BOOST_REQUIRE_EQUAL( core_from_string("700.0000"), get_balance( "alice1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( stake_to_self_with_transfer, aacio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( stake_while_pending_refund, aacio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, aacio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "aacio", "alice1111111", core_from_string("2000.0000"), core_from_string("1000.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( stake_from_refund, aacio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( stake_to_another_user_not_from_refund, aacio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( producer_register_unregister, aacio_system_tester ) try {
   ...
   auto key2 =  fc::crypto::public_key( std::string("AAC5jnmSKrzdBHE9n8hw58y7yxFWBC8SNiG7m8S1crJH3KvAnf9o6") );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(alice1111111), N(regproducer), mvo()
                                               ("producer",  "alice1111111")
                                               ("producer_key", key2 )
                                               ("url", "http://block.two")
...
BOOST_FIXTURE_TEST_CASE( vote_for_producer, aacio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   cross_15_percent_threshold();
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_from_string("2.0000"), core_from_string("0.0002")/*"2.0000 AAC", "0.0002 AAC"*/ ) );
   ...
   //carol1111111 unstakes rest of aac
...
BOOST_FIXTURE_TEST_CASE( unregistered_producer_voting, aacio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   issue( "bob111111111", core_from_string("2000.0000"),  config::system_account_name );
...
BOOST_FIXTURE_TEST_CASE( producer_keep_votes, aacio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   issue( "alice1111111", core_from_string("1000.0000"),  config::system_account_name );
   fc::variant params = producer_parameters_example(1);
...
BOOST_FIXTURE_TEST_CASE( vote_for_two_producers, aacio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   //alice1111111 becomes a producer
   fc::variant params = producer_parameters_example(1);
...
BOOST_FIXTURE_TEST_CASE( proxy_actions_affect_producers, aacio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE(producer_pay, aacio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      const int64_t  initial_savings           = get_balance(N(aacio.saving)).get_amount();
      ...
      const int64_t  savings           = get_balance(N(aacio.saving)).get_amount();
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, aacio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...

   {
      ...
      const uint32_t initial_tot_unpaid_blocks = initial_global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    initial_supply            = get_token_supply();
      const asset    initial_bpay_balance      = get_balance(N(aacio.bpay));
      const asset    initial_vpay_balance      = get_balance(N(aacio.vpay));
      ...
      const uint32_t tot_unpaid_blocks = global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    supply            = get_token_supply();
      const asset    bpay_balance      = get_balance(N(aacio.bpay));
      const asset    vpay_balance      = get_balance(N(aacio.vpay));
...

   {
      ...
      BOOST_REQUIRE_EQUAL( error("missing authority of aacio"),
...
BOOST_FIXTURE_TEST_CASE(producers_upgrade_system_contract, aacio_system_tester) try {
   ...
   string aacio_system_wast2 = aacio_system_wast;
   string msg = "producer votes must be unique and sorted";
   auto pos = aacio_system_wast2.find(msg);
   BOOST_REQUIRE( pos != std::string::npos );
   msg[0] = 'P';
   aacio_system_wast2.replace( pos, msg.size(), msg );

   transaction trx;
   {
      auto code = wast_to_wasm( aacio_system_wast2 );
...
BOOST_FIXTURE_TEST_CASE(producer_onblock_check, aacio_system_tester) try {
...
BOOST_FIXTURE_TEST_CASE( voters_actions_affect_proxy_and_producers, aacio_system_tester, * boost::unit_test::tolerance(1e+6) ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, aacio_system_tester ) try {
   create_accounts_with_resources( {  N(defproducer1), N(defproducer2), N(defproducer3) } );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer1", 1) );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer2", 2) );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer3", 3) );

   //stake more than 15% of total AAC supply to activate chain
   transfer( "aacio", "alice1111111", core_from_string("600000000.0000"), "aacio" );
...
BOOST_FIXTURE_TEST_CASE( buyname, aacio_system_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(fail) }, N(dan) ), // dan shouldn't be able to create fail
                            aacio_assert_message_exception, aacio_assert_message_is( "no active bid for name" ) );
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(nofail) }, N(dan) ), // dan shoudn't be able to do this, sam won
                            aacio_assert_message_exception, aacio_assert_message_is( "only highest bidder can claim" ) );
   ...
   transfer( "aacio", "nofail", core_from_string( "1000.0000" ) );
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(test.fail) }, N(dan) ), // dan shouldn't be able to do this
                            aacio_assert_message_exception, aacio_assert_message_is( "only suffix may create this account" ) );

   create_accounts_with_resources( { N(goodgoodgood) }, N(dan) ); /// 12 char names should succeed
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( bid_invalid_names, aacio_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, aacio_system_tester ) try {
   ...
   {
      const asset initial_names_balance = get_balance(N(aacio.names));
      BOOST_REQUIRE_EQUAL( success(),
      ...
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_from_string("0.1001"), get_balance(N(aacio.names)) );
   }
...
BOOST_FIXTURE_TEST_CASE( vote_producers_in_and_out, aacio_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( setparams, aacio_system_tester ) try {
   //install multisig contract
   abi_serializer msig_abi_ser = initialize_multisig();
   auto producer_names = active_and_vote_producers();
   ...
   aacio::chain::chain_config params;
...
BOOST_FIXTURE_TEST_CASE( setram_effect, aacio_system_tester ) try {
