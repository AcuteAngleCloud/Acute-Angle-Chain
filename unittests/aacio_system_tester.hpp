...
namespace aacio_system {

class aacio_system_tester : public TESTER {
public:

   aacio_system_tester()
   : aacio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   aacio_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(aacio.token), N(aacio.ram), N(aacio.ramfee), N(aacio.stake),
               N(aacio.bpay), N(aacio.vpay), N(aacio.saving), N(aacio.names) });
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("aacio")  + get_balance("aacio.ramfee") + get_balance("aacio.stake") + get_balance("aacio.ram") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& aacin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",aacin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(aacio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(aacio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "aacio", "aacio.msig", core_from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "aacio", "alice1111111", core_from_string("650000000.0000"), "aacio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::aacio_code_name}, 1},
...
inline uint64_t M( const string& aac_str ) {
   return core_from_string( aac_str ).get_amount();
}
