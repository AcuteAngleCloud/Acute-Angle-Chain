...
const static uint64_t null_account_name      = N(aacio.null);
const static uint64_t producers_account_name = N(aacio.prods);
...
const static uint64_t aacio_code_name = N(aacio.code);

const static int      block_interval_ms = 500*2;
...
constexpr uint64_t AAC_PERCENT(uint64_t value, uint32_t percentage) {
