...
#define N(X) aacio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<aacio::chain::name> : private hash<uint64_t> {
      typedef aacio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const aacio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, aacio::chain::name& check);
} // fc


FC_REFLECT( aacio::chain::name, (value) )
