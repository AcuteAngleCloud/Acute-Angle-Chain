#pragma once
#include <aacio/chain/block_header.hpp>
...
FC_REFLECT_ENUM( aacio::chain::transaction_receipt::status_enum,
                 (executed)(soft_fail)(hard_fail)(delayed)(expired) )

FC_REFLECT(aacio::chain::transaction_receipt_header, (status)(cpu_usage_us)(net_usage_words) )
FC_REFLECT_DERIVED(aacio::chain::transaction_receipt, (aacio::chain::transaction_receipt_header), (trx) )
FC_REFLECT_DERIVED(aacio::chain::signed_block, (aacio::chain::signed_block_header), (transactions)(block_extensions) )
