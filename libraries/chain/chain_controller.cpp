...
#include <aacio/chain/action_objects.hpp>
...
#include <aacio/chain/permission_link_object.hpp>
#include <aacio/chain/authority_checker.hpp>
#include <aacio/chain/contracts/chain_initializer.hpp>
#include <aacio/chain/scope_sequence_object.hpp>
#include <aacio/chain/merkle.hpp>

#include <aacio/chain/exceptions.hpp>
#include <aacio/chain/wasm_interface.hpp>

#include <aacio/utilities/rand.hpp>
...
} AAC_CAPTURE_AND_RETHROW( transaction_exception ) }
...
{ try {
   ...
   if( should_check_authorization() ) {
      ...
      AAC_ASSERT( mtrx.delay >= enforced_delay,
...
void chain_controller::_finalize_block( const block_trace& trace, const producer_object& signing_producer ) { try {
   ...
   _resource_limits.set_block_parameters(
      {AAC_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct), chain_config.max_block_cpu_usage, config::block_cpu_usage_average_window_ms / config::block_interval_ms, 1000, {99, 100}, {1000, 999}},
      {AAC_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, 1000, {99, 100}, {1000, 999}}
   );
...
{ try {
   ...
   try {
      ...
      if( !_pending_block ) {
         _start_pending_block();   //开始等待块
      }

      _finalize_pending_cycle();   //完成等待周期
...
{ try {
   ...
   AAC_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
static void validate_shard_locks(const vector<shard_lock>& locks, const string& tag) {
   ...
   for (auto cur = locks.begin() + 1; cur != locks.end(); ++cur) {
      auto prev = cur - 1;
      AAC_ASSERT(*prev != *cur, block_lock_exception, "${tag} lock \"${a}::${s}\" is not unique", ("tag",tag)("a",cur->account)("s",cur->scope));
      AAC_ASSERT(*prev < *cur,  block_lock_exception, "${tag} locks are not sorted", ("tag",tag));
   }
}
...
{ try {
   ...
   for( uint32_t region_index = 0; region_index < next_block.regions.size(); ++region_index ) {
      ...
      AAC_ASSERT(!r.cycles_summary.empty(), tx_empty_region,"region[${r_index}] has no cycles", ("r_index",region_index));
      for (uint32_t cycle_index = 0; cycle_index < r.cycles_summary.size(); cycle_index++) {
         ...
         AAC_ASSERT(!cycle.empty(), tx_empty_cycle,"region[${r_index}] cycle[${c_index}] has no shards", ("r_index",region_index)("c_index",cycle_index));
         for (uint32_t shard_index = 0; shard_index < cycle.size(); shard_index++) {
            const auto& shard = cycle.at(shard_index);
            AAC_ASSERT(!shard.empty(), tx_empty_shard,"region[${r_index}] cycle[${c_index}] shard[${s_index}] is empty",
            ...
            for (const auto& s: shard.read_locks) {
               AAC_ASSERT(write_locks.count(s) == 0, block_concurrency_exception,
            ...
            for (const auto& s: shard.write_locks) {
               ...
               AAC_ASSERT(read_locks.count(s) == 0, block_concurrency_exception,
            ...
            for (const auto& receipt : shard.transactions) {
                ...
                auto make_metadata = [&]() -> transaction_metadata* {
                  ...
                  if( itr != trx_index.end() ) {
                     ...
                     if( should_check_authorization() ) {
                        ...
                        AAC_ASSERT( trx_meta.delay >= enforced_delay,
                                    transaction_exception,
                                    "authorization imposes a delay (${enforced_delay} sec) greater than the delay specified in transaction header (${specified_delay} sec)",
               ...
               AAC_ASSERT( receipt.status == s_trace.transaction_traces.back().status, tx_receipt_inconsistent_status,
               ...
               AAC_ASSERT( receipt.kcpu_usage == s_trace.transaction_traces.back().kcpu_usage, tx_receipt_inconsistent_cpu,
               ...
               AAC_ASSERT( receipt.net_usage_words == s_trace.transaction_traces.back().net_usage_words, tx_receipt_inconsistent_net,
            ...
            AAC_ASSERT( boost::equal( used_read_locks, shard.read_locks ),
               block_lock_exception, "Read locks for executing shard: ${s} do not match those listed in the block", ("s", shard_index));
            AAC_ASSERT( boost::equal( used_write_locks, shard.write_locks ),
   ...
   for (const auto& act : trx.actions ) {
      for (const auto& declared_auth : act.authorization) {
         if (!checker.satisfied(declared_auth)) {
            AAC_ASSERT(checker.satisfied(declared_auth), tx_missing_sigs,
                                                        ...
                                                        flat_set<permission_level> provided_levels)const
{
         ...
         if( act.account == config::system_account_name ) {
            ...
            if( act.name == contracts::linkauth::get_name() ) {
               ...
               if( declared_auth.actor == link.account ) {
                  ...
                  if( linked_permission_name.valid() && *linked_permission_name != config::aacio_any_name ) {
               ...
               if( declared_auth.actor == unlink.account ) {
                  ...
                  if( unlinked_permission_name.valid() && *unlinked_permission_name != config::aacio_any_name ) {
   ...
   if( !allow_unused_signatures && should_check_signatures() ) {
      AAC_ASSERT( checker.all_keys_used(), tx_irrelevant_sig,
...
#warning TODO: this comment sounds like it is expecting a check ("may") somewhere else, but I have not found anything else
   ...
   try {
      ...
      if( *linked_permission == config::aacio_any_name )
         return optional<permission_name>();
...
void chain_controller::validate_uniqueness( const transaction& trx )const {
   ...
   AAC_ASSERT(transaction == nullptr, tx_duplicate, "Transaction is not unique");
}
...
void chain_controller::record_transaction(const transaction& trx)
{
   try {
       ...
       AAC_ASSERT( false, transaction_exception,
...
static uint32_t calculate_transaction_cpu_usage( const transaction_trace& trace, const transaction_metadata& meta, const chain_config& chain_configuration ) {
   ...
   AAC_ASSERT( cpu_usage_limit == 0 || actual_cpu_usage <= cpu_usage_limit, tx_resource_exhausted,
...
static uint32_t calculate_transaction_net_usage( const transaction_trace& trace, const transaction_metadata& meta, const chain_config& chain_configuration ) {
   ...
   AAC_ASSERT( net_usage_limit == 0 || actual_net_usage <= net_usage_limit, tx_resource_exhausted,
...
void chain_controller::update_resource_usage( transaction_trace& trace, const transaction_metadata& meta ) {
   ...
   AAC_ASSERT(trace.cpu_usage <= chain_configuration.max_transaction_cpu_usage,
   ...
   AAC_ASSERT(trace.net_usage <= chain_configuration.max_transaction_net_usage,
...
{ try {
   ...
   AAC_ASSERT( now < time_point(trx.expiration),
...
{ try {
   ...
   AAC_ASSERT( time_point(trx.expiration) <= reference_time + fc::seconds(chain_configuration.max_transaction_lifetime),
...
{ try {
   AAC_ASSERT( !trx.actions.empty(), tx_no_action, "transaction must have at least one action" );
   ...
   AAC_ASSERT( has_auth, tx_no_auths, "transaction must have at least one authorization" );

   // Check that there are no authorizations in any of the context-free actions
   for (const auto &act : trx.context_free_actions) {
      AAC_ASSERT( act.authorization.empty(), cfa_irrelevant_auth, "context-free actions cannot require authorization" );
   }

   AAC_ASSERT( trx.max_kcpu_usage.value < UINT32_MAX / 1024UL, transaction_exception, "declared max_kcpu_usage overflows when expanded to max cpu usage" );
   AAC_ASSERT( trx.max_net_usage_words.value < UINT32_MAX / 8UL, transaction_exception, "declared max_net_usage_words overflows when expanded to max net usage" );
...
{ try {
   ...
   AAC_ASSERT( net_usage_limit == 0 || min_net_usage <= net_usage_limit,
               transaction_exception,
...
void chain_controller::require_scope( const scope_name& scope )const {
   switch( uint64_t(scope) ) {
      case config::aacio_all_scope:
      case config::aacio_auth_scope:
...
const producer_object& chain_controller::validate_block_header(uint32_t skip, const signed_block& next_block)const { try {
   AAC_ASSERT(head_block_id() == next_block.previous, block_validate_exception, "",
              ("head_block_id",head_block_id())("next.prev",next_block.previous));
   AAC_ASSERT(head_block_time() < (fc::time_point)next_block.timestamp, block_validate_exception, "",
   ...
   if( !is_start_of_round( next_block.block_num() ) )  {
      AAC_ASSERT(!next_block.new_producers, block_validate_exception,
   ...
   if(!(skip&skip_producer_signature))
      AAC_ASSERT(next_block.validate_signee(producer.signing_key), block_validate_exception,
   ...
   if(!(skip&skip_producer_schedule_check)) {
      AAC_ASSERT(next_block.producer == producer.owner, block_validate_exception,
   ...
   AAC_ASSERT( next_block.schedule_version == expected_schedule_version , block_validate_exception,"wrong producer schedule version specified ${x} expectd ${y}",
...
void chain_controller::_update_producers_authority() {
   const auto& gpo = get_global_properties();
   uint32_t authority_threshold = AAC_PERCENT_CEIL(gpo.active_producers.producers.size(), config::producers_authority_threshold_pct);
...
} AAC_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission: ${level}", ("level", level) ) }
...
ProducerRound chain_controller::calculate_next_round(const signed_block& next_block) {
   ...
   AAC_ASSERT(boost::range::equal(next_block.producer_changes, changes), block_validate_exception,
...
void chain_controller::update_last_irreversible_block()
{
   ...
   size_t offset = AAC_PERCENT(producer_objs.size(), config::percent_100- config::irreversible_threshold_percent);
