...
void apply_aacio_newaccount(apply_context& context) {
   ...
   AAC_ASSERT( validate(create.recovery), action_validate_exception, "Invalid recovery authority");
   ...
   AAC_ASSERT(existing_account == nullptr, action_validate_exception,
...
void apply_aacio_setabi(apply_context& context) {
   ...
   if ( act.account == aacio::chain::config::system_account_name ) {
      act.abi = chain_initializer::aac_contract_abi(act.abi);
   }
...
void apply_aacio_updateauth(apply_context& context) {
   ...
   AAC_ASSERT(validate(update.data), action_validate_exception,
   ...
   auto permission_is_valid_for_update = [&](){
      ...
      AAC_ASSERT(current != nullptr, permission_query_exception,
...
void apply_aacio_onerror(apply_context& context) {
...
static const abi_serializer& get_abi_serializer() {
   ...
   if (!_abi_serializer) {
      _abi_serializer.emplace(chain_initializer::aac_contract_abi(abi_def()));
   }
...
void apply_aacio_postrecovery(apply_context& context) {
   context.require_write_lock( config::aacio_auth_scope );
...
void apply_aacio_passrecovery(apply_context& context) {
...
void apply_aacio_vetorecovery(apply_context& context) {
   context.require_write_lock( config::aacio_auth_scope );
