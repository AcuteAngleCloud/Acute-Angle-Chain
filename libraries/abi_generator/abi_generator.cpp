...
void abi_generator::handle_decl(const Decl* decl) { try {
  ...
  // If AACIO_ABI macro was found, check if the current declaration
  ...
  // The current Decl was the type referenced in AACIO_ABI macro
  if( type_has_actions ) return;

  // The current Decl was not the type referenced in AACIO_ABI macro
