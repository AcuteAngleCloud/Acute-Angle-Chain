#!/bin/bash
#
# aacio-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# aacio-tn_bounce.sh and aacio-tn_roll.sh scripts.

connected="0"

rundir=programs/nodaac
prog=nodaac
...
datadir=var/lib/node_$AACIO_NODE
...
relaunch() {
    echo "$rundir/$prog $* --data-dir $datadir --config-dir etc/aacio/node_$AACIO_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $* --data-dir $datadir --config-dir etc/aacio/node_$AACIO_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
