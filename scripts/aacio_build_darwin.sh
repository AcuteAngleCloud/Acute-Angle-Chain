	...
	if [ "${OS_MIN}" -lt 12 ]; then
		echo "You must be running Mac OS 10.12.x or higher to install AACIO."
		echo "Exiting now."
		exit 1
	fi
	...
	then
		printf "\\tHomebrew must be installed to compile AAC.IO\\n\\n"
	...
	do
		...
		(( COUNT++ ))
	done < "${SOURCE_DIR}/scripts/aacio_build_dep"
	IFS="${var_ifs}"
	...
	if [ "${BVERSION}" != "106700" ]; then
		if [ ! -z "${BVERSION}" ]; then
			...
			printf "\\tAAC.IO requires Boost version 1.67.\\n"
