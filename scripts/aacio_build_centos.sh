	...
	if [ $OS_VER -lt 7 ]; then
		echo "You must be running Centos 7 or higher to install AACIO."
		echo "exiting now"
		exit 1
	fi

	if [ ${DISK_AVAIL%.*} -lt $DISK_MIN ]; then
		echo "You must have at least ${DISK_MIN}GB of available storage to install AACIO."
		echo "exiting now"
		exit 1
	fi
	...
	if [ $? -ne 0 ]; then
		printf "\n\tYum must be installed to compile AAC.IO.\n"
		printf "\n\tExiting now.\n"
		exit 0
	fi
	...
	if [ -z $SCL ]; then
		printf "\n\tThe Centos Software Collections Repository, devtoolset-7 and Python3 are required to install AACIO.\n"
	...
	if [ ! -d ${HOME}/opt/binaryen ]; then
		...
		cd ${TEMP_DIR}
		git clone https://github.com/AACIO/binaryen
		cd binaryen
		git checkout aacio
		$CMAKE . && make
		if [ $? -ne 0 ]; then
