...
#include <aaciolib/vector.hpp>

namespace identity {
   using aacio::action_meta;
   ...
   class contract {
      public:
         ...
         struct create : public action_meta< code, N(create) >
         {
            ...
            AACLIB_SERIALIZE( create, (creator)(identity) )
         };
         ...
         struct certvalue {
            ...
            AACLIB_SERIALIZE( certvalue, (property)(type)(data)(memo)(confidence) )
         };
         ...
         struct certprop : public action_meta< code, N(certprop) >
         {
            ...
            AACLIB_SERIALIZE( certprop, (bill_storage_to)(certifier)(identity)(values) )
         };
         ...
         struct settrust : public action_meta< code, N(settrust) >
         {
            ...
            AACLIB_SERIALIZE( settrust, (trustor)(trusting)(trust) )
         };
         ...
         struct certrow {
            ...
            AACLIB_SERIALIZE( certrow , (property)(trusted)(certifier)(confidence)(type)(data)(id) )
         };
         ...
         struct identrow {
            ...
            AACLIB_SERIALIZE( identrow , (identity)(creator) )
         };
         ...
         struct trustrow {
            ...
            AACLIB_SERIALIZE( trustrow, (account) )
         };

         typedef aacio::multi_index<N(certs), certrow,
                                    aacio::indexed_by< N(bytuple), aacio::const_mem_fun<certrow, key256, &certrow::get_key> >
                                    > certs_table;
         typedef aacio::multi_index<N(ident), identrow> idents_table;
         typedef singleton<code, N(account), code, identity_name>  accounts_table;
         typedef aacio::multi_index<N(trust), trustrow> trust_table;
         ...
         static bool is_trusted( account_name acnt ) {
            account_name active_producers[31];
         ...
         static void on( const create& c ) {
            ...
            aacio_assert( itr == t.end(), "identity already exists" );
            aacio_assert( c.identity != 0, "identity=0 is not allowed" );
         ...
         static void on( const certprop& cert ) {
            ...
            idents_table t( code, code );
            aacio_assert( t.find( cert.identity ) != t.end(), "identity does not exist" );
            ...
            for( const auto& value : cert.values ) {
               auto idx = certs.template get_index<N(bytuple)>();
               if (value.confidence) {
                  aacio_assert(value.type.size() <= 32, "certrow::type should be not longer than 32 bytes");
                  ...
                  if (value.property == N(owner)) {
                     aacio_assert(sizeof(account_name) == value.data.size(), "data size doesn't match account_name size");
         ...
         static void apply( account_name c, action_name act) {
            aacio::dispatch<contract, create, certprop, settrust>(c,act);
         }
