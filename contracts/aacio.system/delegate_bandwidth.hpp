   ...
   class delegate_bandwidth : public voting<SystemAccount> {
      public:
         ...
         struct total_resources {
            ...
            AACLIB_SERIALIZE( total_resources, (owner)(net_weight)(cpu_weight)(storage_stake)(storage_bytes) )
         };
         ...
         struct delegated_bandwidth {
            ...
            AACLIB_SERIALIZE( delegated_bandwidth, (from)(to)(net_weight)(cpu_weight)(storage_stake)(storage_bytes) )
         ...
         struct refund_request {
            ...
            AACLIB_SERIALIZE( refund_request, (owner)(request_time)(amount) )
         };

         typedef aacio::multi_index< N(totalband), total_resources>   total_resources_table;
         ...
         ACTION( SystemAccount, delegatebw ) {
            ...
            AACLIB_SERIALIZE( delegatebw, (from)(receiver)(stake_net_quantity)(stake_cpu_quantity)(stake_storage_quantity) )
         };
         ...
         ACTION( SystemAccount, undelegatebw ) {
            ...
            AACLIB_SERIALIZE( undelegatebw, (from)(receiver)(unstake_net_quantity)(unstake_cpu_quantity)(unstake_storage_bytes) )
         };

         ACTION( SystemAccount, refund ) {
            account_name owner;

            AACLIB_SERIALIZE( refund, (owner) )
         };

         static void on( const delegatebw& del ) {
            aacio_assert( del.stake_cpu_quantity.amount >= 0, "must stake a positive amount" );
            aacio_assert( del.stake_net_quantity.amount >= 0, "must stake a positive amount" );
            aacio_assert( del.stake_storage_quantity.amount >= 0, "must stake a positive amount" );

            system_token_type total_stake = del.stake_cpu_quantity + del.stake_net_quantity + del.stake_storage_quantity;
            aacio_assert( total_stake.quantity > 0, "must stake a positive amount" );

            require_auth( del.from );

            //aacio_assert( is_account( del.receiver ), "can only delegate resources to an existing account" );
            uint64_t storage_bytes = 0;
            if ( 0 < del.stake_storage_quantity.amount ) {
               ...
               aacio_assert( 0 < storage_bytes, "stake is too small to increase storage even by 1 byte" );
         ...
         static void on( account_name receiver, const undelegatebw& del ) {
            aacio_assert( del.unstake_cpu_quantity.amount >= 0, "must unstake a positive amount" );
            aacio_assert( del.unstake_net_quantity.amount >= 0, "must unstake a positive amount" );
            ...
            aacio_assert( dbw.net_weight >= del.unstake_net_quantity, "insufficient staked net bandwidth" );
            aacio_assert( dbw.cpu_weight >= del.unstake_cpu_quantity, "insufficient staked cpu bandwidth" );
            aacio_assert( dbw.storage_bytes >= del.unstake_storage_bytes, "insufficient staked storage" );
            ...
            aacio_assert( total_refund.quantity > 0, "must unstake a positive amount" );
