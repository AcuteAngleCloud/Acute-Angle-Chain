...
namespace aaciosystem {
   ...
   struct user_resources {
      ...
      AACLIB_SERIALIZE( user_resources, (owner)(net_weight)(cpu_weight)(ram_bytes)(is_sys_create)(is_sys_ram)(is_sys_net) )
   };
   ...
   struct delegated_bandwidth {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      AACLIB_SERIALIZE( delegated_bandwidth, (from)(to)(net_weight)(cpu_weight) )
   ...
   struct refund_request {
      account_name  owner;
      time          request_time;
      aacio::asset  net_amount;
      aacio::asset  cpu_amount;

      uint64_t  primary_key()const { return owner; }

      // explicit serialization macro is not necessary, used here only to improve compilation time
      AACLIB_SERIALIZE( refund_request, (owner)(request_time)(net_amount)(cpu_amount) )
   };
    ...
    */
   typedef aacio::multi_index< N(userres), user_resources>      user_resources_table;
   typedef aacio::multi_index< N(delband), delegated_bandwidth> del_bandwidth_table;
   typedef aacio::multi_index< N(refunds), refund_request>      refunds_table;
   ...
   {
      require_auth( payer );
      aacio_assert( quant.amount > 0, "must purchase a positive amount" );

      if (payer == N(aacio.cname)) {
      ...
      INLINE_ACTION_SENDER(aacio::token, transfer)( N(aacio.token), {payer,N(active)},
         { payer, N(aacio.ram), quant_after_fee, std::string("buy ram") } );

      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(aacio::token, transfer)( N(aacio.token), {payer,N(active)},
                                                       { payer, N(aacio.ramfee), fee, std::string("ram fee") } );
      }
      ...
      aacio_assert( bytes_out > 0, "must reserve a positive amount" );
      ...
      if( res_itr ==  userres.end() ) {
         ...
         userres.modify( res_itr, receiver, [&]( auto& res ) {
               res.ram_bytes += bytes_out;
               if ( (payer == N(aacio.cname)) && (res_itr->is_sys_create) && (!res_itr->is_sys_ram) ) {
                  res.is_sys_ram = true;
               }
            });
      }
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      aacio_assert( bytes > 0, "cannot sell negative byte" );

      user_resources_table  userres( _self, account );
      auto res_itr = userres.find( account );
      aacio_assert( res_itr != userres.end(), "no resource row" );
      aacio_assert( res_itr->ram_bytes >= bytes, "insufficient quota" );
      ...
      aacio_assert( _gstate.total_ram_stake >= 0, "error, attempt to unstake more tokens than previously staked" );
      ...
      INLINE_ACTION_SENDER(aacio::token, transfer)( N(aacio.token), {N(aacio.ram),N(active)},
                                                       { N(aacio.ram), account, asset(tokens_out), std::string("sell ram") } );
   ...
   {
      require_auth( from );
      aacio_assert( stake_net_delta != asset(0) || stake_cpu_delta != asset(0), "should stake non-zero amount" );
      aacio_assert( std::abs( (stake_net_delta + stake_cpu_delta).amount )
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         aacio_assert( asset(0) <= itr->net_weight, "insufficient staked net bandwidth" );
         aacio_assert( asset(0) <= itr->cpu_weight, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         if( tot_itr ==  totals_tbl.end() ) {
            ...
            totals_tbl.modify( tot_itr, from == receiver ? from : 0, [&]( auto& tot ) {
                  tot.net_weight    += stake_net_delta;
                  tot.cpu_weight    += stake_cpu_delta;
                  if ( (from == N(aacio.cname)) && (tot_itr->is_sys_create) && (!tot_itr->is_sys_net) ) {
                	  tot.is_sys_net = true;
                  }
               });
         }
         aacio_assert( asset(0) <= tot_itr->net_weight, "insufficient staked total net bandwidth" );
         aacio_assert( asset(0) <= tot_itr->cpu_weight, "insufficient staked total cpu bandwidth" );
      ...
      if ( N(aacio.stake) != source_stake_from ) { //for aacio both transfer and refund make no sense
         ...
         if( is_delegating_to_self || is_undelegating ) {
            if ( req != refunds_tbl.end() ) { //need to update refund
               ...
               aacio_assert( asset(0) <= req->net_amount, "negative net refund amount" ); //should never happen
               aacio_assert( asset(0) <= req->cpu_amount, "negative cpu refund amount" ); //should never happen
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(aacio::token, transfer)( N(aacio.token), {source_stake_from, N(active)},
               { source_stake_from, N(aacio.stake), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
      ...
      // update voting power
      {
         ...
         aacio_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == N(b1) ) {
   ...
   {
      aacio_assert( stake_cpu_quantity >= asset(0), "must stake a positive amount" );
      aacio_assert( stake_net_quantity >= asset(0), "must stake a positive amount" );
      aacio_assert( stake_net_quantity + stake_cpu_quantity > asset(0), "must stake a positive amount" );
      aacio_assert( !transfer || from != receiver, "cannot use transfer flag if delegating to self" );
      if (from == N(aacio.cname)) {
         user_resources_table  userres( _self, receiver );
         auto res_itr = userres.find( receiver );
         if( res_itr !=  userres.end()) {
            aacio_assert( (res_itr->is_sys_create) && (!res_itr->is_sys_net), "system account refuse to delegatebw for receiver" );
            aacio_assert( stake_cpu_quantity < asset(5000), "system account must stake less then 0.5" );
            aacio_assert( stake_net_quantity < asset(5000), "system account must stake less then 0.5" );
         }
      }
   ...
   {
      aacio_assert( asset() <= unstake_cpu_quantity, "must unstake a positive amount" );
      aacio_assert( asset() <= unstake_net_quantity, "must unstake a positive amount" );
      aacio_assert( asset() < unstake_cpu_quantity + unstake_net_quantity, "must unstake a positive amount" );
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      aacio_assert( req != refunds_tbl.end(), "refund request not found" );
      aacio_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
      // Until now() becomes NOW, the fact that now() is the timestamp of the previous block could in theory
      // allow people to get their tokens earlier than the 3 day delay if the unstake happened immediately after many
      // consecutive missed blocks.

      INLINE_ACTION_SENDER(aacio::token, transfer)( N(aacio.token), {N(aacio.stake),N(active)},
                                                    { N(aacio.stake), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
