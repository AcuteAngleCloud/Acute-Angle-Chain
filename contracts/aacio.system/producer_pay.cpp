...
namespace aaciosystem {
   ...
   void system_contract::claimrewards( const account_name& owner ) {
      ...
      aacio_assert( ct - prod.last_claim_time > useconds_per_day, "already claimed rewards within past day" );
      ...
      if( usecs_since_last_fill > 0 && _gstate.last_pervote_bucket_fill > 0 ) {
         ...
         INLINE_ACTION_SENDER(aacio::token, transfer)( N(aacio.token), {N(aacio),N(active)},
                                                       { N(aacio), N(aacio.saving), asset(to_savings), "unallocated inflation" } );
      ...
      if( producer_per_block_pay > 0 ) {
         INLINE_ACTION_SENDER(aacio::token, transfer)( N(aacio.token), {N(aacio.bpay),N(active)},
                                                       { N(aacio.bpay), owner, asset(producer_per_block_pay), std::string("producer block pay") } );
      }
      if( producer_per_vote_pay > 0 ) {
         INLINE_ACTION_SENDER(aacio::token, transfer)( N(aacio.token), {N(aacio.vpay),N(active)},
                                                       { N(aacio.vpay), owner, asset(producer_per_vote_pay), std::string("producer vote pay") } );
      }
   }
