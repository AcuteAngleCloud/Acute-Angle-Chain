...
namespace stltest {
   ...
   struct MSTR {
    ...
    class contract {
    public:
        typedef aacio::token<N(mycurrency),S(4,MYCUR)> token_type;
       ...
       static void f(const char* __restrict, ...) {
        ...
        static void on( const message& ) {
           ...
           */
           ...
           prints("\nAAC string: "); prints_l(s2.get_data(), s2.get_size());
           */
        ...
        static void apply( account_name c, action_name act) {
            aacio::dispatch<stltest::contract, message>(c,act);
        }
    };
