...
void test_transaction::assert_false_error_handler(const aacio::deferred_transaction& dtrx) {
   auto onerror_action = aacio::get_action(1, 0);
   aacio_assert( onerror_action.authorization.at(0).actor == dtrx.actions.at(0).account,
...
void test_transaction::send_transaction_expiring_late(uint64_t receiver, uint64_t, uint64_t) {
   ...
   aacio_assert(false, "send_transaction_expiring_late() should've thrown an error");
}
...
void test_transaction::read_inline_action() {
   ...
   aacio_assert(res == -1, "get_action error 0");
   ...
   aacio_assert(res != -1, "get_action error");
   ...
   aacio_assert(dres.a == 1 && dres.b == 2 && dres.c == 3, "data_as error");

   res = get_action( 3, 1, buffer, 64);
   aacio_assert(res == -1, "get_action error -1");
}
