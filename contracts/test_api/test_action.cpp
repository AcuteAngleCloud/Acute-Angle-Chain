...
void test_action::test_dummy_action() {
   ...
   aacio_assert( act.authorization.back().actor == N(testapi), "incorrect permission actor" );
   aacio_assert( act.authorization.back().permission == N(active), "incorrect permission name" );
...
void test_action::test_cf_action() {
   ...
   if ( cfa.payload == 100 ) {
      ...
      float f3 = f1 + f2;
      aacio_assert( f3 >  2.0f, "Unable to add float.");
      // verify compiler builtin api access
      __int128 ret;
      __divti3(ret, 2, 2, 2, 2);
      // verify context_free_system_api
      aacio_assert( true, "verify aacio_assert can be called" );
      ...
      aacio::require_auth(N(test));
      aacio_assert( false, "authorization_api should not be allowed" );
   } else if ( cfa.payload == 207 ) {
      now();
      aacio_assert( false, "system_api should not be allowed" );
      ...
      aacio_assert( false, "transaction_api should not be allowed" );
...
void test_action::test_publication_time() {
   ...
   aacio_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)");
...
void test_action::test_current_time() {
   ...
   aacio_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)");
   aacio_assert( tmp == current_time(), "tmp == current_time()" );
}
...
void test_action::test_ram_billing_in_notify(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   aacio_assert( total == sizeof(uint128_t), "total == sizeof(uint128_t)");

   uint64_t to_notify = tmp >> 64;
   uint64_t payer = tmp & 0xFFFFFFFFFFFFFFFFULL;

   if( code == receiver ) {
      aacio::require_recipient( to_notify );
   } else {
      aacio_assert( to_notify == receiver, "notified recipient other than the one specified in to_notify" );
