...
using aac_currency = aaciosystem::contract<N(aacio)>::currency;
...
class dice : public aacio::contract {
   public:
      ...
      void offerbet(const asset& bet, const account_name player, const checksum256& commitment) {
         ...
         auto amount = aac_currency::token_type(bet);

         aacio_assert( amount.quantity > 0, "invalid bet" );
         ...
         if( matched_offer_itr == idx.end()
            ...
            accounts.modify( cur_player_itr, 0, [&](auto& acnt) {
               acnt.aac_balance = (asset)(aac_currency::token_type(acnt.aac_balance) - amount);
               acnt.open_offers++;
            });
      ...
      void canceloffer( const checksum256& commitment ) {
         ...
         accounts.modify(acnt_itr, 0, [&](auto& acnt){
            acnt.open_offers--;
            acnt.aac_balance.amount += offer_itr->bet.amount;
         });
      ...
      void claimexpired( const uint64_t gameid ) {
         ...
         aacio_assert(game_itr->deadline != 0 && now() > game_itr->deadline, "game not expired");
      ...
      void deposit( const account_name from, const asset& a ) {
         ...
         auto amount = aac_currency::token_type(a);

         aac_currency::inline_transfer( from, _self, amount );
      ...
      void withdraw( const account_name to, const asset& a ) {
         ...
         aac_currency::inline_transfer( _self, to, amount );
   ...
   private:
      ...
      void pay_and_clean(const game& g, const offer& winner_offer,
         ...
         accounts.modify( winner_account, 0, [&]( auto& acnt ) {
            acnt.aac_balance.amount += 2*g.bet.amount;
            acnt.open_games--;
         });
