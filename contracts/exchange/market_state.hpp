...
namespace aacio {
   ...
   struct margin_position {
      ...
      AACLIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef aacio::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), aacio::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      AACLIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef aacio::multi_index<N(loans), loan_position> loans;
