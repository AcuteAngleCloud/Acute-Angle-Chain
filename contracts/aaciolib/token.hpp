...
namespace aacio {
  ...
  */
  ...
  struct token {
    /**
    * Type of the currency (e.g. aac) represented as an unsigned 64 bit integer
    ...
    */
    ...
    token( const asset& a ):quantity(NumberType(a.amount)) {
       aacio_assert( a.symbol == Symbol, "attempt to construct token from asset with different symbol" );
       aacio_assert( 0 <= a.amount, "attemt to convert asset with negative value to token" );
    }
    ...
    */
    token& operator-=( const token& a ) {
      aacio_assert( quantity >= a.quantity, "integer underflow subtracting token balance" );
      quantity -= a.quantity;
      return *this;
    }
    ...
    */
    token& operator+=( const token& a ) {
      aacio_assert( quantity + a.quantity >= a.quantity, "integer overflow adding token balance" );
      quantity += a.quantity;
      return *this;
    }
    ...
    */
    token& operator*=( uint64_t a ) {
      aacio_assert( a == 0 || (quantity * a) / a == quantity, "integer overflow multiplying token balance" );
      quantity *= a;
      return *this;
    }
  ...
  struct price
  {
    ...
    */
    price( BaseToken base, QuoteToken quote ) {
      aacio_assert( base  >= BaseToken(1ul), "invalid price" );
      aacio_assert( quote >= QuoteToken(1ul), "invalid price" );
    ...
    */
    ...
    AACLIB_SERIALIZE( price, (base_per_quote) )
  private:
    /**
