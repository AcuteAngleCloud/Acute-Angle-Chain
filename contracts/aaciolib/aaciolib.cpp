...
namespace aacio {
   ...
   void set_blockchain_parameters(const aacio::blockchain_parameters& params) {
      char buf[sizeof(aacio::blockchain_parameters)];
      aacio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(aacio::blockchain_parameters& params) {
      char buf[sizeof(aacio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      aacio_assert( size <= sizeof(buf), "buffer is too small" );
      aacio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   */
   ...
   private:
      ...
      public:
         ...
         {
            aacio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return aacio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = aacio::memory_heap.malloc(count*size);
...
{
   return aacio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return aacio::memory_heap.free(ptr);
}
