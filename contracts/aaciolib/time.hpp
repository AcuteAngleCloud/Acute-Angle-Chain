...
namespace aacio {
  class microseconds {
    public:
        ...
        int64_t _count;
        AACLIB_SERIALIZE( microseconds, (_count) )
    private:
  ...
  class time_point {
    public:
        ...
        AACLIB_SERIALIZE( time_point, (elapsed) )
  };
  ...
  class time_point_sec
  {
    public:
        ...
        operator time_point()const { return time_point( aacio::seconds( utc_seconds) ); }
        uint32_t sec_since_epoch()const { return utc_seconds; }

        time_point_sec operator = ( const aacio::time_point& t )
        {
          utc_seconds = uint32_t(t.time_since_epoch().count() / 1000000ll);
          return *this;
        }
        ...
        AACLIB_SERIALIZE( time_point_sec, (utc_seconds) )
  };
   ...
   class block_timestamp {
      public:
         ...
         block_timestamp next() const {
            aacio_assert( std::numeric_limits<uint32_t>::max() - slot >= 1, "block timestamp overflow" );
         ...
         bool   operator !=( const block_timestamp& t )const   { return slot != t.slot; }
         uint32_t slot;
         static constexpr int32_t block_interval_ms = 500*4; //block produced per 2 seconds,update by ck 2018-7-11
         static constexpr int64_t block_timestamp_epoch = 946684800000ll;  // epoch is year 2000

         AACLIB_SERIALIZE( block_timestamp, (slot) )
      private:
