...
namespace aacio {
   ...
   public:
      ...
      AACLIB_SERIALIZE( transaction_header, (expiration)(region)(ref_block_num)(ref_block_prefix)(net_usage_words)(kcpu_usage)(delay_sec) )
   };
   ...
   public:
      ...
      AACLIB_SERIALIZE_DERIVED( transaction, transaction_header, (context_free_actions)(actions) )
   };
   ...
   class deferred_transaction : public transaction {
      public:
         ...
         AACLIB_SERIALIZE_DERIVED( deferred_transaction, transaction, (sender_id)(sender)(payer)(execute_after) )
   };
   ...
   inline action get_action( uint32_t type, uint32_t index ) {
      ...
      aacio_assert( size > 0, "get_action size failed" );
      char buf[size];
      auto size2 = ::get_action(type, index, &buf[0], static_cast<size_t>(size) );
      aacio_assert( size == size2, "get_action failed" );
      return aacio::unpack<aacio::action>(&buf[0], static_cast<size_t>(size));
   }
