#include <aacio.msig/aacio.msig.hpp>
#include <aaciolib/action.hpp>
#include <aaciolib/permission.hpp>
...
void multisig::propose() {
   ...
   aacio_assert( trx_header.expiration >= aacio::time_point_sec(now()), "transaction expired" );
   //aacio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   aacio_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   aacio_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   aacio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   aacio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      aacio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < aacio::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
AACIO_ABI( aacio::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
