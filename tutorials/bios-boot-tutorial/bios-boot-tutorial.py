...
fastUnstakeSystem = './fast.refund/aacio.system/aacio.system.wasm'
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.claac + 'create account aacio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.claac + 'system newaccount --transfer aacio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.claac + 'transfer aacio %s "%s"' % (a['name'], intToCurrency(unstaked)))

def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(args.claac + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])

def listProducers():
    run(args.claac + 'system listproducers')
...
def claimRewards():
    table = getJsonOutput(args.claac + 'get table aacio aacio producers -l 100')
    times = []
...
def proxyVotes(b, e):
    ...
    retry(args.claac + 'system regproxy ' + proxy)
    sleep(1.0)
    for i in range(b, e):
        voter = accounts[i]['name']
        retry(args.claac + 'system voteproducer proxy ' + voter + ' ' + proxy)

def updateAuth(account, permission, parent, controller):
    run(args.claac + 'push action aacio updateauth' + jsonArg({
        'account': account,
...
def resign(account, controller):
    ...
    sleep(1)
    run(args.claac + 'get account ' + account)
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.claac + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    trxPermissions = [{'actor': 'aacio', 'permission': 'active'}]
    with open(fastUnstakeSystem, mode='rb') as f:
        setcode = {'account': 'aacio', 'vmtype': 0, 'vmversion': 0, 'code': f.read().hex()}
    run(args.claac + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'aacio setcode' + jsonArg(setcode) + ' -p ' + proposer)

def msigApproveReplaceSystem(proposer, proposalName):
    for i in range(firstProducer, firstProducer + numProducers):
        run(args.claac + 'multisig approve ' + proposer + ' ' + proposalName +
            jsonArg({'actor': accounts[i]['name'], 'permission': 'active'}) +
            '-p ' + accounts[i]['name'])

def msigExecReplaceSystem(proposer, proposalName):
    retry(args.claac + 'multisig exec ' + proposer + ' ' + proposalName + ' -p ' + proposer)

def msigReplaceSystem():
    run(args.claac + 'push action aacio buyrambytes' + jsonArg(['aacio', accounts[0]['name'], 200000]) + '-p aacio')
    sleep(1)
...
def stepKillAll():
    run('killall kaacd nodaac || true')
    sleep(1.5)
def stepStartWallet():
    startWallet()
    importKeys()
def stepStartBoot():
    startNode(0, {'name': 'aacio', 'pvt': args.private_key, 'pub': args.public_key})
    sleep(1.5)
def stepInstallSystemContracts():
    run(args.claac + 'set contract aacio.token ' + args.contracts_dir + 'aacio.token/')
    run(args.claac + 'set contract aacio.msig ' + args.contracts_dir + 'aacio.msig/')
def stepCreateTokens():
    run(args.claac + 'push action aacio.token create \'["aacio", "10000000000.0000 %s"]\' -p aacio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.claac + 'push action aacio.token issue \'["aacio", "%s", "memo"]\' -p aacio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.claac + 'set contract aacio ' + args.contracts_dir + 'aacio.system/')
    sleep(1)
    run(args.claac + 'push action aacio setpriv' + jsonArg(['aacio.msig', 1]) + '-p aacio@active')
...
def stepLog():
    run('tail -n 60 ' + args.nodes_dir + '00-aacio/stderr')
...
commands = [
    ('k', 'kill',           stepKillAll,                True,    "Kill all nodaac and kaacd processes"),
    ('w', 'wallet',         stepStartWallet,            True,    "Start kaacd, create wallet, fill with keys"),
    ('b', 'boot',           stepStartBoot,              True,    "Start boot node"),
    ('s', 'sys',            createSystemAccounts,       True,    "Create system accounts (aacio.*)"),
    ...
    ('q', 'resign',         stepResign,                 True,    "Resign aacio"),
...
parser.add_argument('-H', '--http-port', type=int, default=8000, metavar='', help='HTTP port for claac')
