...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.aacioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    if testWallet is None:
        cmdError("aac wallet create")
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.aacioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.aacioAccount, stakedDeposit=0, waitForTransBlock=False)
    if transId is None:
        cmdError("%s create account" % (testeraAccount.name))
        errorExit("Failed to create account %s" % (testeraAccount.name))

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.aacioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.aacioAccount, stakedDeposit=5000)
    if transId is None:
        cmdError("%s create account" % (ClientName))
        errorExit("Failed to create account %s" % (currencyAccount.name))

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.aacioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.aacioAccount, waitForTransBlock=True)
    if transId is None:
        cmdError("%s create account" % (ClientName))
    ...
    actualAmount=node.getAccountAacBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountAacBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    contractDir="contracts/aacio.token"
    wastFile="contracts/aacio.token/aacio.token.wast"
    abiFile="contracts/aacio.token/aacio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getAacAccountFromDb(currencyAccount.name)
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get AAC account failed", raw=True)
    ...
    account=node.getAacAccount(defproduceraAccount.name)
    if account is None:
