...
class Utils:
    ...
    AacClientPath="programs/claac/claac"

    AacWalletName="kaacd"
    AacWalletPath="programs/kaacd/"+ AacWalletName

    AacServerName="nodaac"
    AacServerPath="programs/nodaac/"+ AacServerName
    ...
    # mongoSyncTime: nodaac mongodb plugin seems to sync with a 10-15 seconds delay. This will inject
...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="AACtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getAacAccount(account.name)
            try:
                assert(accountInfo)
    ...
    def getBlock(self, blockNum, retry=True, silentErrors=False):
        ...
        if not self.enableMongo:
            cmd="%s %s get block %d" % (Utils.AacClientPath, self.endpointArgs, blockNum)
    ...
    def getTransaction(self, transId, retry=True, silentErrors=False):
        if not self.enableMongo:
            cmd="%s %s get transaction %s" % (Utils.AacClientPath, self.endpointArgs, transId)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                trans=Utils.runCmdReturnJson(cmd)
                return trans
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False):
        cmd='%s %s system newaccount -j %s %s %s %s --stake-net "100 %s" --stake-cpu "100 %s" --buy-ram "100 %s"' % (
            Utils.AacClientPath, self.endpointArgs, creatorAccount.name, account.name,
    ...
    def getAacAccount(self, name):
        assert(isinstance(name, str))
        cmd="%s %s get account -j %s" % (Utils.AacClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def getAacAccountFromDb(self, name):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def getTable(self, contract, scope, table):
        cmd="%s %s get table %s %s %s" % (Utils.AacClientPath, self.endpointArgs, contract, scope, table)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL):
        ...
        cmd="%s %s get currency balance %s %s %s" % (Utils.AacClientPath, self.endpointArgs, contract, account, symbol)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def getCurrencyStats(self, contract, symbol=CORE_SYMBOL):
        ...
        assert(symbol)
        assert(isinstance(symbol, str))
        cmd="%s %s get currency stats %s %s" % (Utils.AacClientPath, self.endpointArgs, contract, symbol)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def verifyAccount(self, account):
        if not self.enableMongo:
            ret=self.getAacAccount(account.name)
            if ret is not None:
            ...
            for _ in range(2):
                ret=self.getAacAccountFromDb(account.name)
                if ret is not None:
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.AacClientPath, self.endpointArgs, source.name, destination.name)
        cmdArr=cmd.split()
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        currentBalances=self.getAacBalances([source] + accounts)
    ...
    def getAacBalances(self, accounts):
        ...
        balances={}
        for account in accounts:
            balance = self.getAccountAacBalance(account.name)
    ...
    def getAccountsByKey(self, key):
        cmd="%s %s get accounts %s" % (Utils.AacClientPath, self.endpointArgs, key)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    # Get actions mapped to an account (claac get actions)
    def getActions(self, account, pos=-1, offset=-1):
        assert(isinstance(account, Account))
        assert(isinstance(pos, int))
        assert(isinstance(offset, int))

        cmd="%s %s get actions -j %s %d %d" % (Utils.AacClientPath, self.endpointArgs, account.name, pos, offset)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def getServants(self, name):
        cmd="%s %s get servants %s" % (Utils.AacClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def getAccountAacBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from claac get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        if not self.enableMongo:
            amount=self.getTableAccountBalance("aacio.token", scope)
            if Utils.Debug: Utils.Print("getNodeAccountAacBalance %s %s" % (scope, amount))
            ...
            account=self.getAacAccountFromDb(scope)
            if account is not None:
                balance=account["aac_balance"]
                return balance

        return None

    def getAccountAacBalance(self, scope):
        """Returns SYS currency0000 account balance from claac get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccountAacBalanceStr(scope)
        balance=Node.currencyStrToInt(balanceStr)
        return balance

    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.AacClientPath, self.endpointArgs, account)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            retStr=Utils.checkOutput(cmd.split())
    ...
    def publishContract(self, account, contractDir, wastFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd="%s %s -v set contract -j %s %s" % (Utils.AacClientPath, self.endpointArgs, account, contractDir)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd="%s %s push action -j %s %s" % (Utils.AacClientPath, self.endpointArgs, account, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False):
        cmd="%s %s set action permission -j %s %s %s %s" % (
            Utils.AacClientPath, self.endpointArgs, account, code, pType, requirement)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
            trans=Utils.runCmdReturnJson(cmd)
    ...
    def getInfo(self, silentErrors=False):
        cmd="%s %s get info" % (Utils.AacClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by claac get info."""
        if not self.enableMongo:
            info=self.getInfo()
            if info is not None:
...
class WalletMgr(object):
    __walletLogFile="test_kaacd_output.log"
    __walletDataDir="test_wallet_0"

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(kaacd) process; False=Manage launch process externally.
    def __init__(self, walletd, nodaacPort=8888, nodaacHost="localhost", port=8899, host="localhost"):
        self.walletd=walletd
        self.nodaacPort=nodaacPort
        self.nodaacHost=nodaacHost
        self.port=port
        self.host=host
        self.wallets={}
        self.__walletPid=None
        self.endpointArgs="--url http://%s:%d" % (self.nodaacHost, self.nodaacPort)
    ...
    def create(self, name, accounts=None):
        ...
        if wallet is not None:
            ...
            return wallet
        p = re.compile(r'\n\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet create --name %s" % (Utils.AacClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=Utils.checkOutput(cmd.split())
    ...
    def importKey(self, account, wallet):
        ...
        cmd="%s %s wallet import --name %s %s" % (
            Utils.AacClientPath, self.endpointArgs, wallet.name, account.ownerPrivateKey)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
        ...
        if account.activePrivateKey is None:
            ...
            cmd="%s %s wallet import --name %s %s" % (
                Utils.AacClientPath, self.endpointArgs, wallet.name, account.activePrivateKey)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                Utils.checkOutput(cmd.split())
            except subprocess.CalledProcessError as ex:
    ...
    def lockWallet(self, wallet):
        cmd="%s %s wallet lock --name %s" % (Utils.AacClientPath, self.endpointArgs, wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def unlockWallet(self, wallet):
        cmd="%s %s wallet unlock --name %s" % (Utils.AacClientPath, self.endpointArgs, wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def lockAllWallets(self):
        cmd="%s %s wallet lock_all" % (Utils.AacClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
    ...
    def getOpenWallets(self):
        ...
        cmd="%s %s wallet list" % (Utils.AacClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=Utils.checkOutput(cmd.split())
    ...
    def getKeys(self, wallet):
        ...
        p = re.compile(r'\n\s+\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet private_keys --name %s --password %s " % (Utils.AacClientPath, self.endpointArgs, wallet.name, wallet.password)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=Utils.checkOutput(cmd.split())
        #Utils.Print("retStr: %s" % (retStr))
        m=p.findall(retStr)
    ...
    def killall(self, allInstances=False):
        """Kill kaac instances. allInstances will kill all kaac instances running on the system."""
        if self.__walletPid:
            Utils.Print("Killing wallet manager process %d:" % (self.__walletPid))
            os.kill(self.__walletPid, signal.SIGKILL)

        if allInstances:
            cmd="pkill -9 %s" % (Utils.AacWalletName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
...
class Cluster(object):
    ...
    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kaacd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=8899, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="AACtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kaacd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: aac server host
        port: aac server port
        walletHost: aac wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures aac mongo plugin
        ...
        self.aacioAccount=Account("aacio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontKill=False, dontBootstrap=False, totalProducers=None):
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.AacServerName))
            return True
        ...
        cmd="%s -p %s -n %s -s %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.AacLauncherPath, pnodes, totalNodes, topo, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            p2pPlugin, producerFlag)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodaacArgs="--max-transaction-time 5000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodaacArgs += " --plugin aacio::wallet_api_plugin"
        if self.enableMongo:
            nodaacArgs += " --plugin aacio::mongo_db_plugin --delete-all-blocks --mongodb-uri %s" % self.mongoUri

        if nodaacArgs:
            cmdArr.append("--nodaac")
            cmdArr.append(nodaacArgs)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.AacServerName, totalNodes, len(nodes)))
            return False
        ...
        initAccountKeys(self.aacioAccount, producerKeys["aacio"])
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key" % (Utils.AacClientPath)
                if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on aac server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextAacIdx: %d, n: %d" % (nextAacIdx, n))
                nextAacIdx=(nextAacIdx + 1)%count
                if not self.nodes[nextAacIdx].killed:
                    #Utils.Print("nextAacIdx: %d" % (nextAacIdx))
            ...
            #Utils.Print("nextAacIdx: %d, count: %d" % (nextAacIdx, count))
            node=self.nodes[nextAacIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.AacServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on aac node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getAacBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.aacioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        node="node_bios"
        configFile="etc/aacio/%s/config.ini" % (node)
        if Utils.Debug: Utils.Print("Parsing config file %s" % configFile)
        producerKeys=Cluster.parseProducerKeys(configFile, node)
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse aacio private keys from cluster config files.")
            return None
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        ...
        try:
            ...
            aacioName="aacio"
            aacioKeys=producerKeys[aacioName]
            aacioAccount=Account(aacioName)
            aacioAccount.ownerPrivateKey=aacioKeys["private"]
            aacioAccount.ownerPublicKey=aacioKeys["public"]
            aacioAccount.activePrivateKey=aacioKeys["private"]
            aacioAccount.activePublicKey=aacioKeys["public"]

            if not walletMgr.importKey(aacioAccount, ignWallet):
                Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (aacioName))
                return False
            ...
            trans=biosNode.publishContract(aacioAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
            ...
            for name, keys in producerKeys.items():
                ...
                trans=biosNode.createAccount(initx, aacioAccount, 0)
                if trans is None:
            ...
            if not onlyBios:
                if prodCount == -1:
                    ...
                    with open(setProdsFile, "r") as f:
                        ...
                        myTrans=biosNode.pushMessage("aacio", "setprods", setProdsStr, opts)
                    ...
                    trans=biosNode.pushMessage("aacio", "setprods", setProdsStr, opts)
                ...
                # wait for block production handover (essentially a block produced by anyone but aacio).
                lam = lambda: biosNode.getInfo()["head_block_producer"] != "aacio"
                ret=Utils.waitForBool(lam)
                if not ret:
                    Utils.Print("ERROR: Block production handover failed.")
                    return False

            aacioTokenAccount=copy.deepcopy(aacioAccount)
            aacioTokenAccount.name="aacio.token"
            trans=biosNode.createAccount(aacioTokenAccount, aacioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (aacioTokenAccount.name))
                return False

            aacioRamAccount=copy.deepcopy(aacioAccount)
            aacioRamAccount.name="aacio.ram"
            trans=biosNode.createAccount(aacioRamAccount, aacioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (aacioRamAccount.name))
                return False

            aacioRamfeeAccount=copy.deepcopy(aacioAccount)
            aacioRamfeeAccount.name="aacio.ramfee"
            trans=biosNode.createAccount(aacioRamfeeAccount, aacioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (aacioRamfeeAccount.name))
                return False

            aacioStakeAccount=copy.deepcopy(aacioAccount)
            aacioStakeAccount.name="aacio.stake"
            trans=biosNode.createAccount(aacioStakeAccount, aacioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (aacioStakeAccount.name))
                return False
            ...
            contract="aacio.token"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(aacioTokenAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return False

            # Create currency0000, followed by issue currency0000
            contract=aacioTokenAccount.name
            Utils.Print("push create action to %s contract" % (contract))
            action="create"
            data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (aacioTokenAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push create action to aacio contract.")
                return False
            ...
            action="issue"
            data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (aacioAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push issue action to aacio contract.")
                return False
            ...
            Utils.Print("Verify aacio issue, Expected: %s" % (expectedAmount))
            actualAmount=biosNode.getAccountAacBalanceStr(aacioAccount.name)
            if expectedAmount != actualAmount:
                Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                            (expectedAmount, actualAmount))
                return False

            contract="aacio.system"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            ...
            action="transfer"
            for name, keys in producerKeys.items():
                data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (aacioAccount.name, name, initialFunds, "init transfer")
                opts="--permission %s@active" % (aacioAccount.name)
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (aacioTokenAccount.name, name))
                    return False
    ...
    # Populates list of AacInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=0):
        ...
        cmd="pgrep %s %s" % (pgrepOpts, Utils.AacServerName)
        ...
        for i in range(0, totalNodes):
            ...
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.AacServerName, pattern))
                break
    ...
    # Kills a percentange of Aac instances starting from the tail and update aacInstanceInfos state
    def killSomeAacInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.AacServerName, killSignal))
    ...
    def dumpErrorDetails(self):
        fileName="etc/aacio/node_bios/config.ini"
        Cluster.dumpErrorDetailImpl(fileName)
        fileName="var/lib/node_bios/stderr.txt"
        Cluster.dumpErrorDetailImpl(fileName)

        for i in range(0, len(self.nodes)):
            fileName="etc/aacio/node_%02d/config.ini" % (i)
            Cluster.dumpErrorDetailImpl(fileName)
            fileName="var/lib/node_%02d/stderr.txt" % (i)
            Cluster.dumpErrorDetailImpl(fileName)

    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodaac instances. allInstances will kill all nodaac instances running on the system."""
        cmd="%s -k 9" % (Utils.AacLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down aac cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the aac server
            cmd="pkill -9 %s" % (Utils.AacServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down aac cluster.")
    ...
    def cleanup(self):
        for f in glob.glob("var/lib/node_*"):
            shutil.rmtree(f)
        for f in glob.glob("etc/aacio/node_*"):
            shutil.rmtree(f)
...
class TestState(object):
    ...
    def __init__(self, *ignoreArgs):
        ...
        if "--host" not in self.__ignoreArgs:
            self.parser.add_argument("-h", "--host", type=str, help="%s host name" % (Utils.AacServerName),
                                     default=self.__LOCAL_HOST)
        if "--port" not in self.__ignoreArgs:
            self.parser.add_argument("-p", "--port", type=int, help="%s host port" % Utils.AacServerName,
        ...
        if "--clean-run" not in self.__ignoreArgs:
            self.parser.add_argument("--clean-run", help="Kill all nodaac and klaac instances", action='store_true')
    ...
    def start(self, cluster):
        ...
        if self.localTest and not self.__dontLaunch:
            ...
            if self.cluster.launch(prodCount=self.__prodCount, dontKill=self.__dontKill, pnodes=self.pnodes, totalNodes=self.totalNodes, totalProducers=self.totalProducers) is False:
                ...
                Utils.errorExit("Failed to stand up aac cluster.")
