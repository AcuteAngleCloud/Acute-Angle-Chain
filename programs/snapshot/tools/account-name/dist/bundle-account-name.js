...
const lookup = value => {
  setTimeout(() => {
    ...
    if (startsWithETH(lookup)) found = SNAPSHOT.findIndex(row => row.aac == lookup);else if (startsWithAAC(lookup)) found = SNAPSHOT.findIndex(row => row.eth == lookup);else display_invalid();
