...
const update_title_bar_progress = (stage = "") => {
  ...
  switch( stage ){
    ...
    case 'distribute':
      title = `AAC | Snapshot ${debug.rates.percent_complete}% | ${debug.registry.accepted+debug.registry.rejected}/${debug.registry.total}`
    break
    case 'registry':
      title = `AAC | Building Registry | ${registrants.length}`
    break
    case 'reclaimer':
      title = `AAC | Reclaimable TXs | ${ debug.reclaimable.total }`
    break
    case 'complete':
      title = `AAC | Snapshot Complete`
    break
    default:
      title = `AAC | Welcome`
    break
