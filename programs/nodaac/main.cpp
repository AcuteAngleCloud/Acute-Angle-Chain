/**
 *  @file
 *  @copyright defined in aacio/LICENSE.txt
 */
...
{
   try {
      app().set_version(aacio::nodaac::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "aacio/nodaac/data" );
      app().set_default_config_dir(root / "aacio/nodaac/config" );
      if(!app().initialize<chain_plugin, http_plugin, net_plugin, producer_plugin>(argc, argv))
         return INITIALIZE_FAIL;
      initialize_logging();
      ilog("nodaac version ${ver}", ("ver", aacio::utilities::common::itoh(static_cast<uint32_t>(app().version()))));
      ilog("aacio root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
