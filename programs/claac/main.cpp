...
fc::variant call( const std::string& server, uint16_t port,
                  ...
                  const T& v ) { return aacio::client::http::call( server, port, path, fc::variant(v) ); }

template<typename T>
fc::variant call( const std::string& path,
                  const T& v ) { return aacio::client::http::call( host, port, path, fc::variant(v) ); }

aacio::chain_apis::read_only::get_info_results get_info() {
  return call(host, port, get_info_func ).as<aacio::chain_apis::read_only::get_info_results>();
}
...
void send_transaction( signed_transaction& trx, int32_t extra_kcpu, packed_transaction::compression_type compression = packed_transaction::none  ) {
   ...
   if( tx_print_json ) {
      ...
      auto trace = result["processed"].as<aacio::chain::transaction_trace>();
      print_result( result );
   }
}
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active) {
   return action {
      ...
      contracts::newaccount{
         ...
         .recovery     = aacio::chain::authority{1, {}, {{{creator, config::active_name}, 1}}}
      }
   };
}
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->set_callback([this] {
         ...
         if (is_delete) {
            ...
            authority auth;
            if (boost::istarts_with(authorityJsonOrFile, "AAC")) {
               try {
...
int main( int argc, char** argv ) {
   ...
   app.add_option( "-H,--host", host, localized("the host where nodaac is running"), true );
   app.add_option( "-p,--port", port, localized("the port where nodaac is running"), true );
   app.add_option( "--wallet-host", wallet_host, localized("the host where kaacd is running"), true );
   app.add_option( "--wallet-port", wallet_port, localized("the port where kaacd is running"), true );
   ...
   try {
      ...
      if (errorString.find("Connection refused") != string::npos) {
         if (errorString.find(fc::json::to_string(port)) != string::npos) {
            std::cerr << localized("Failed to connect to nodaac at ${ip}:${port}; is nodaac running?", ("ip", host)("port", port)) << std::endl;
         } else if (errorString.find(fc::json::to_string(wallet_port)) != string::npos) {
            std::cerr << localized("Failed to connect to kaacd at ${ip}:${port}; is kaacd running?", ("ip", wallet_host)("port", wallet_port)) << std::endl;
         } else {
