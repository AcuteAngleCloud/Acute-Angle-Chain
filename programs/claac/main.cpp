...
fc::variant call( const std::string& url,
   ...
   try {
      ...
      return aacio::client::http::do_http_call( *cp, fc::variant(v), print_request, print_response );
   }
...
void ensure_kaacd_running(CLI::App* app) {
    ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append("kaacd"); // if claac and kaacd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kaacd").append("kaacd");
    }
