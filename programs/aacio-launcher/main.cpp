...
#include <net/if.h>
#include <aacio/chain/genesis_state.hpp>
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = aacio::net_plugin\n";
  } else {
    cfg << "plugin = aacio::bnet_plugin\n";
  }
  cfg << "plugin = aacio::chain_api_plugin\n"
      << "plugin = aacio::history_api_plugin\n";
  cfg.close();
}
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      aacio::chain::genesis_state default_genesis;
...
launcher_def::launch (aacd_def &instance, string &gts) {
  ...
  string aacdcmd = "programs/nodaac/nodaac ";
