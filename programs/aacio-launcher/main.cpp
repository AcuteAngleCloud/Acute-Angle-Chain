...
void
aacd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string aacio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), aacio_home(), instances(1) {}
};
...
struct launcher_def {
  ...
  producer_set_def producer_set;
   ...
   void assign_name (aacd_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (aacd_def &node, host_def *host);
  void launch (aacd_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, aacd_def> find_node(uint16_t node_num);
  vector<pair<host_def, aacd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("producers",bpo::value<size_t>(&producers)->default_value(31),"total number of non-bios producer instances in this network")
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodaac does not require transaction signatures.")
    ("nodaac", bpo::value<string>(&aacd_extra_args), "forward nodaac command line argument(s) to each instance of nodaac, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodaac instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &aacd : binding.instances) {
          aacd.host = binding.host_name;
          aacd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(aacd.p2p_port);

          aliases.push_back (aacd.name);
        }
      }
  ...
  char* erd_env_var = getenv ("AACIO_HOME");
...
void
launcher_def::assign_name (aacd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.aacio_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      aacd_def aacd;
      assign_name(aacd, i == 0);
      aliases.push_back(aacd.name);
      aacd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(aacd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->aacio_home =
          (local_id.contains (lhost->host_name) || servers.default_aacio_home.empty()) ?
          erd : servers.default_aacio_home;
        host_ndx++;
      } // ph_count == 0

      aacd_def aacd;
      assign_name(aacd, do_bios);
      aacd.has_db = false;
      ...
      aliases.push_back(aacd.name);
      aacd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(aacd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::bind_nodes () {
   ...
   for (auto &h : bindings) {
      for (auto &inst : h.instances) {
         ...
         node.instance = &inst;
         auto kp = is_bios ?
            private_key_type(string("5Jgm1N6jp3iNbFM45kPtj66xmbqT9fSuVJgPWfnCSPgQanvW6mJ")) :
            private_key_type::generate();
         auto pubkey = kp.get_public_key();
         node.keys.emplace_back (move(kp));
         if (is_bios) {
            string prodname = "aacio";
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  aacd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->aacio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->aacio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->aacio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->aacio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->aacio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = aacio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = aacio::mongo_db_plugin\n";
  }
  cfg << "plugin = aacio::chain_api_plugin\n"
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      aacio::chain::contracts::genesis_state_type default_genesis;
...
void
launcher_def::prep_remote_config_dir (aacd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->aacio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->aacio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->aacio_home;

  cmd = "cd " + host->aacio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->aacio_home
...
void
launcher_def::launch (aacd_def &instance, string &gts) {
  ...
  if (!aacd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +aacio::mongo_db_plugin");
       string args = std::regex_replace (aacd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       aacdcmd += args + " ";
    }
    else {
       aacdcmd += aacd_extra_args + " ";
    }
  }

  if( add_enable_stale_production ) {
    aacdcmd += "--enable-stale-production true ";
  ...
  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->aacio_home + "; nohup " + aacdcmd + " > "
    ...
    string cmd = "cd " + host->aacio_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << aacdcmd << endl;

    bp::child c(aacdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << aacdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(aacd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, aacd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, aacd_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const aacd_def& node = node_pair.second;
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the aacio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the aacio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the aacio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << aacio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(aacio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (aacio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_aacio_home) (ssh) )
