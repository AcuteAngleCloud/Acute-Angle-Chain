 ...
 */
#include <aacio/wallet_plugin/wallet_plugin.hpp>
...
void wallet_plugin::set_program_options(options_description& cli, options_description& cfg) {
   cfg.add_options()
         ...
         ("aacio-key", bpo::value<std::string>(),
          "aacio key that will be imported automatically when a wallet is created.")
         ;
}
...
void wallet_plugin::plugin_initialize(const variables_map& options) {
   ...
   if (options.count("aacio-key")) {
      std::string aacio_wif_key = options.at("aacio-key").as<std::string>();
      aacio_wif_key = fc::json::from_string(aacio_wif_key).as<std::string>();
      wallet_manager_ptr->set_aacio_key(aacio_wif_key);
   }
}
} // namespace aacio
