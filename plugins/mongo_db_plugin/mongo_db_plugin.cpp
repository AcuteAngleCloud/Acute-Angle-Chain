   ...
   try {
      ...
      if( account ) {
         ...
         try {
            ...
            ilog( "Unable to convert AAC JSON to MongoDB JSON: ${e}", ("e", e.what()));
            ilog( "  AAC JSON: ${j}", ("j", json));
      ...
      if (act.name != "onblock") { // aacio::onblock not in original aacio.system abi
      ...
      if (actions_to_write) {
         try {
            if( !bulk_actions.execute() ) {
               AAC_ASSERT( false, chain::mongo_db_insert_fail, "Bulk actions insert failed for transaction: ${id}", ("id", trx_id_str));
            }
         } catch(...) {
