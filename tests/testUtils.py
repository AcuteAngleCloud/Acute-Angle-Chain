...
class Utils:
    ...
    def iAmNotNoon():
        ...
        Utils.AacClientPath="programs/aacc/aacc"

        Utils.AacWalletName="aac-walletd"
        Utils.AacWalletPath="programs/aac-walletd/"+ Utils.AacWalletName

        Utils.AacServerName="aacd"
        Utils.AacServerPath="programs/aacd/"+ Utils.AacServerName
...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, alive=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="AACtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def getBlock(self, blockNum, retry=True, silentErrors=False):
        if not self.enableMongo:
            cmd="%s %s get block %s" % (Utils.AacClientPath, self.endpointArgs, blockNum)
    ...
    def createInitAccounts(self, producers):
        ...
        aacio = Account("aacio")
        # aacio = self.aacioAccount

        # publish system contract
        contractDir="contracts/aacio.system"
        wastFile="contracts/aacio.system/aacio.system.wast"
        abiFile="contracts/aacio.system/aacio.system.abi"
        Utils.Print("Publish aacio.system contract")
        trans=self.publishContract(aacio.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
        if trans is None:
           Utils.errorExit("Failed to publish aacio.system.")

        Utils.Print("push issue action to aacio contract")
        contract=aacio.name
        action="issue"
        data="{\"to\":\"aacio\",\"quantity\":\"1000000000.0000 AAC\"}"
        ...
        # Utils.Print("Verify aacio issue, Expected: %d" % (expectedAmount))
        # actualAmount=self.getAccountBalance(aacio.name)
        ...
        for name, keys in producers.items():
            ...
            trans=self.createAccount(initx, aacio, 0)
            if trans is None:
                Utils.errorExit("Failed to create account %s" % (name))

            trans = self.transferFunds(aacio, initx, 10000000000, "init transfer")
            if trans is None:
                Utils.errorExit("Failed to transfer funds from %s to %s." % (aacio.name, name))
    ...
    def createAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False):
        ...
        if Utils.amINoon:
            ...
            cmd="%s %s create account %s %s %s %s" % (Utils.AacClientPath, self.endpointArgs,
    ...
    def getAacAccount(self, name):
        cmd="%s %s get account %s" % (Utils.AacClientPath, self.endpointArgs, name)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def getAacCurrencyBalance(self, name):
        cmd="%s %s get currency balance aacio %s AAC" % (Utils.AacClientPath, self.endpointArgs, name)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            ...
            msg=ex.output.decode("utf-8")
            Utils.Print("ERROR: Exception during get AAC balance. %s" % (msg))
            return None
    ...
    def transferFunds(self, source, destination, amount, memo="memo", force=False):
        cmd="%s %s -v transfer -j %s %s %d" % (
            Utils.AacClientPath, self.endpointArgs, source.name, destination.name, amount)
        cmdArr=cmd.split()
        cmdArr.append(memo)
        if force:
            cmdArr.append("-f")
    ...
    def getAccountBalance(self, name):
        if not self.enableMongo:
            amount=self.getAacCurrencyBalance(name)
            Utils.Debug and Utils.Print("getAacCurrencyBalance %s %s", name, amount)
            ...
            account=self.getAacAccountFromDb(name)
            if account is not None:
                field=account["aac_balance"]
    ...
    def getTransactionsByAccount(self, name):
        cmd="%s %s get transactions -j %s" % (Utils.AacClientPath, self.endpointArgs, name)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def createProducer(self, account, ownerPublicKey, waitForTransBlock=False):
        cmd="%s %s create producer %s %s" % (Utils.AacClientPath, self.endpointArgs, account, ownerPublicKey)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
            trans=Node.runCmdReturnJson(cmd)
    ...
    def getTable(self, account, contract, table):
        cmd="%s %s get table %s %s %s" % (Utils.AacClientPath, self.endpointArgs, account, contract, table)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Node.runCmdReturnJson(cmd)
            return trans
    ...
    def pushMessage(self, contract, action, data, opts, silentErrors=False):
        cmd=None
        if Utils.amINoon:
            cmd="%s %s push action -j %s %s" % (Utils.AacClientPath, self.endpointArgs, contract, action)
        else:
            cmd="%s %s push message %s %s" % (Utils.AacClientPath, self.endpointArgs, contract, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
...
class WalletMgr(object):
    ...
    def __init__(self, walletd, nodaacPort=8888, nodaacHost="localhost", port=8899, host="localhost"):
        ...
        self.endpointArgs="--host %s --port %d" % (self.nodaacHost, self.nodaacPort)
        if self.walletd:
    ...
    def getKeys(self):
        ...
        p = re.compile('\n\s+\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet keys" % (Utils.AacClientPath, self.endpointArgs)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        retStr=subprocess.check_output(cmd.split()).decode("utf-8")
        #Utils.Print("retStr: %s" % (retStr))
        m=p.findall(retStr)
    ...
    def killall(self):
        cmd="pkill %s" % (Utils.AacWalletName)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=8899, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="AACtest", initaPrvtKey=None, initbPrvtKey=None, staging=False):
        self.accounts={}
        self.nodes={}
    ...
    def launch(self, pnodes=1, total_nodes=1, topo="mesh", delay=0):
        ...
        cmd="%s -p %s -n %s -s %s -d %s -f" % (
            Utils.AacLauncherPath, pnodes, total_nodes, topo, delay)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")
        if not self.walletd or self.enableMongo:
            if Utils.amINoon:
                ...
                cmdArr.append("--aacd")
            if not self.walletd:
                cmdArr.append("--plugin aacio::wallet_api_plugin")
            if self.enableMongo:
                if Utils.amINoon:
                    cmdArr.append("--plugin aacio::mongo_db_plugin --resync --mongodb-uri %s" % self.mongoUri)
                else:
                    cmdArr.append("--plugin aacio::db_plugin --mongodb-uri %s" % self.mongoUri)
        s=" ".join(cmdArr)
        Utils.Print("cmd: %s" % (s))
        ...
        if total_nodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.AacServerName, total_nodes, len(nodes)))
            return False
    ...
    def spreadFunds(self, amount=1):
        ...
        to=self.accounts[0]
        Utils.Print("Transfer %d units from account %s to %s on aac server port %d" % (
        ...
        for i in range(0, count):
            ...
            for n in range(0, count):
                ...
                if self.nodes[nextAacIdx].alive:
    ...
    def parseClusterInfo(self, totalNodes):
        ...
        configFile="etc/aacio/node_bios/config.ini"
        ...
        for i in range(0, totalNodes):
            configFile="etc/aacio/node_%02d/config.ini" % (i)
            Utils.Debug and Utils.Print("Parsing config file %s" % configFile)
        ...
        self.aacioAccount.ownerPrivateKey=aacioKeys[1]
        self.aacioAccount.ownerPublicKey=aacioKeys[0]
        self.aacioAccount.activePrivateKey=aacioKeys[1]
        self.aacioAccount.activePublicKey=aacioKeys[0]
    ...
    # Check state of running nodaac process and update AacInstanceInfos
    #def updateAacInstanceInfos(aacInstanceInfos):
    ...
    def killall(self, silent=True):
        cmd="%s -k 15" % (Utils.AacLauncherPath)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            not silent and Utils.Print("Launcher failed to shut down aac cluster.")

        # ocassionally the launcher cannot kill the aac server
        cmd="pkill %s" % (Utils.AacServerName)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            not silent and Utils.Print("Failed to shut down aac cluster.")
