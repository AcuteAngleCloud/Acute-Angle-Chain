...
try:
    ...
    maxRAMValue=1010
    extraNodaacArgs=" %s %d %s %d " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
    if cluster.launch(onlyBios=False, dontKill=dontKill, pnodes=totalNodes, totalNodes=totalNodes, totalProducers=totalNodes, extraNodaacArgs=extraNodaacArgs) is False:
    ...
    if walletMgr.launch() is False:
        ...
        errorExit("Failed to stand up aac walletd.")

    testWallet=walletMgr.create(testWalletName, [cluster.aacioAccount])
    ...
    # create accounts via aacio as otherwise a bid is needed
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.aacioAccount.name))
        trans=nodes[0].createInitializeAccount(account, cluster.aacioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
        transferAmount="70000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.aacioAccount.name, account.name))
        nodes[0].transferFunds(cluster.aacioAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.aacioAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.aacioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.aacioAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.aacioAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    Print("Failed to push create action to aacio contract. sleep for 60 seconds")
                    time.sleep(60)
                time.sleep(1)
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killAacInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
