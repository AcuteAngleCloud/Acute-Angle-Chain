...
my $aac_home = defined $ENV{AAC_HOME} ? $ENV{AAC_HOME} : getcwd;
my $aacd = $aac_home . "/programs/aacd/aacd";
my $aacc = $aac_home . "/programs/aacc/aacc";

my $nodes = defined $ENV{AAC_TEST_RING} ? $ENV{AAC_TEST_RING} : "1";
my $pnodes = defined $ENV{AAC_TEST_PRODUCERS} ? $ENV{AAC_TEST_PRODUCERS} : "1";

my $prods = 31;
my $genesis = "$aac_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of aacd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"AAC84snobCGtpQvTTfVaxvMuxGDd4p2LhtQhbWX1yWAU5yt4tiB55\",\"5Jgm1N6jp3iNbFM45kPtj66xmbqT9fSuVJgPWfnCSPgQanvW6mJ\"]\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$aacc transfer aac inita 10 >> aacc.out 2>> aacc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
