...
###############################################################
# nodaac_run_remote_test
#  Tests remote capability of the nodaac_run_test. Test will setup cluster and pass nodes info to nodaac_run_test. E.g.
#  nodaac_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodaac_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodaac", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killAacInstances, False, False, killAll, dumpErrorDetails)
