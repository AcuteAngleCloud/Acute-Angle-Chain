...
parser.add_argument("-h", "--host", type=str, help="%s host name" % (testUtils.Utils.AacServerName),
                    default=LOCAL_HOST)
parser.add_argument("-p", "--port", type=int, help="%s host port" % testUtils.Utils.AacServerName,
...
walletMgr=testUtils.WalletMgr(True, nodaacPort=DEFAULT_PORT-100)
testSuccessful=False
killAacInstances=not dontKill
killWallet=not dontKill

WalletdName="aac-walletd"
ClientName="aacc"
...
try:
    ...
    # TODO need to update aacio.system contract to use new currency and update claac and chain_plugin for interaction
...
# TODO Currently unable to set producer
    ...
    account=node.getAacAccount(initaAccount.name)
    if account is None:
