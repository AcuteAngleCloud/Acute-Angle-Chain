...
try:
    ...
    transId=node.createInitializeAccount(testeraAccount, cluster.aacioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.aacioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.aacioAccount, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.aacioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.aacioAccount, waitForTransBlock=True, exitOnError=True)
    ...
    wasmFile="aacio.token.wasm"
    abiFile="aacio.token.abi"
    ...
    account=node.getAacAccount(defproduceraAccount.name, exitOnError=True)
