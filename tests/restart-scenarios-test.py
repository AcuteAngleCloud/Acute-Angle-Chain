...
# --kill-count <nodaac instances to kill>
...
try:
    ...
    Print("Stand up AAC wallet kaacd")
    walletMgr.killall(allInstances=killAll)
    walletMgr.cleanup()
    if walletMgr.launch() is False:
        errorExit("Failed to stand up kaacd.")

    accountsCount=total_nodes
    walletName="MyWallet"
    Print("Creating wallet %s if one doesn't already exist." % walletName)
    wallet=walletMgr.create(walletName, [cluster.aacioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    if wallet is None:
    ...
    aacioAccount=cluster.aacioAccount
    ...
    if not cluster.createAccounts(aacioAccount):
    ...
    if cluster.killSomeAacInstances(killCount, killSignal) is False:
        errorExit("Failed to kill Aac instances")
    Print("nodaac instances killed.")
    ...
    if cluster.relaunchAacInstances() is False:
        errorExit("Failed to relaunch Aac instances")
    Print("nodaac instances relaunched.")
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killAacInstances, killAacInstances, keepLogs, killAll, dumpErrorDetails)
