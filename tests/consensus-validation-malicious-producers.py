...
def stageScenario(stagedNodeInfos):
    ...
    for stagedNodeInfo in stagedNodeInfos:
        configPath=os.path.join(stagingDir, "etc/aacio/node_%02d" % (count))
...
killAacInstances= not args.dont-kill
...
def myTest(transWillEnterBlock):
    ...
    try:
        ...
        if cluster.launch(pnodes, total_nodes, topo, delay) is False:
            error("Failed to stand up aac cluster.")
            return False
        ...
        if walletMgr.launch() is False:
            error("Failed to stand up aac walletd.")
            return False
