...
class Node(object):
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getAacAccount(account.name, exitOnError=True)
            try:
    ...
    def getAacAccount(self, name, exitOnError=False):
        assert(isinstance(name, str))
        if not self.enableMongo:
            cmdDesc="get account"
            cmd="%s -j %s" % (cmdDesc, name)
            msg="( getAacAccount(name=%s) )" % (name);
            return self.processCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
        else:
            return self.getAacAccountFromDb(name, exitOnError=exitOnError)

    def getAacAccountFromDb(self, name, exitOnError=False):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def processCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        ...
        cmd="%s %s %s" % (Utils.AacClientPath, self.endpointArgs, cmd)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
