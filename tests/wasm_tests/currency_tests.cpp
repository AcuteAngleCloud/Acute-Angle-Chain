...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      symbol aac(4, "AAC");
      BOOST_REQUIRE_EQUAL(AAC_SYMBOL_VALUE, aac.value());
      BOOST_REQUIRE_EQUAL("4,AAC", aac.to_string());
      BOOST_REQUIRE_EQUAL("AAC", aac.name());
      BOOST_REQUIRE_EQUAL(4, aac.decimals());
   }
   ...
   // default is "4,AAC"
   {
      ...
      BOOST_REQUIRE_EQUAL("AAC", def.name());
   }
   // from string
   {
   ...
   {
      symbol malformed(SY(6,AaC));
      BOOST_REQUIRE_EQUAL(false, malformed.valid());
      BOOST_REQUIRE_EQUAL("AaC", malformed.name());
