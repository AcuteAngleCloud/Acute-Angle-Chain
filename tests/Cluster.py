...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="AACtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        ...
        self.aacioAccount=self.defProducerAccounts["aacio"]= Account("aacio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontKill=False
               , dontBootstrap=False, totalProducers=None, extraNodaacArgs=None):
        """Launch cluster.
        ...
        if not Cluster.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodaac default port.")
            return False
        ...
        nodaacArgs="--max-transaction-time 50000 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodaacArgs += " --plugin aacio::wallet_api_plugin"
        if self.enableMongo:
            nodaacArgs += " --plugin aacio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodaacArgs is not None:
            assert(isinstance(extraNodaacArgs, str))
            nodaacArgs += extraNodaacArgs
        if Utils.Debug:
            nodaacArgs += " --contracts-console"
        ...
        self.aacioAccount=self.defProducerAccounts["aacio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.AacClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.AacClientPath)
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        ...
        try:
            ...
            trans=biosNode.publishContract(aacioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
            if trans is None:
            ...
            if not onlyBios:
                ...
                lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "aacio"
            ...
            abiFile="%s.abi" % (contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(aacioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return None
