...
class WalletMgr(object):
    ...
    def __init__(self, walletd, nodaacPort=8888, nodaacHost="localhost", port=9899, host="localhost"):
    ...
    def create(self, name, accounts=None, exitOnError=True):
        ...
        if wallet is not None:
            ...
            return wallet
        p = re.compile(r'\n\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet create --name %s --to-console" % (Utils.AacClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
