...
public:
    ...
    {
         return get_currency_balance(N(aacio.token), symbol(SY(4,AAC)), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        //  aacio.msig
        //  aacio.token
        create_accounts({N(aacio.msig), N(aacio.token)});

        // Set code for the following accounts:
        //  aacio.system  (code: aacio.bios)
        //  aacio.msig (code: aacio.msig)
        //  aacio.token    (code: aacio.token)
// These contracts are still under dev
#if _READY
        set_code_abi(N(aacio.msig), aacio_msig_wast, aacio_msig_abi);
#endif
//        set_code_abi(config::system_account_name, aacio_bios_wast, aacio_bios_abi);
        set_code_abi(N(aacio.token), aacio_token_wast, aacio_token_abi);

        ilog(".");
        // Set privileges for aacio.msig
        ...
        auto expected = asset::from_string("1000000000.0000 AAC");
        // Create AAC tokens in aacio.token, set its manager as aacio.system
        create_currency(N(aacio.token), config::system_account_name, expected);

        ilog(".");

        // Issue the genesis supply of 1 billion AAC tokens to aacio.system
        // Issue the genesis supply of 1 billion AAC tokens to aacio.system
        issue(N(aacio.token), config::system_account_name, config::system_account_name, expected); 
        ...
        ilog(".");
        // Transfer AAC to genesis accounts
        for (auto gen_acc : gen_accounts) {
            auto quantity = "10000.0000 AAC";
            auto stake_quantity = "5000.0000 AAC";

            ilog(".");
            auto trace = base_tester::push_action(N(aacio.token), N(transfer), config::system_account_name, mutable_variant_object()
        ...
        // Set code aacio.system from aacio.bios to aacio.system
