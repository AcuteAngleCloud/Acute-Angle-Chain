...
getHeadBlockNum()
{
  INFO="$(programs/claac/claac get info)"
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/claac/claac --port $port2 get block 1)"
    verifyErrorCode "claac get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/claac/claac create key)"
...
PASSWORD_INITA="$(programs/claac/claac wallet create --name inita)"
...
ACCOUNT_INFO="$(programs/claac/claac create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "claac create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/claac/claac get account testera)"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/claac/claac transfer inita testera 975321 "test transfer")"
   ...
   TRANS_INFO="$(programs/claac/claac --port $port2 get transaction $TRANS_ID)"
